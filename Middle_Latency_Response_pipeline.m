...
%% Set Brainstorm nogui on completely headless mode for MLR 
...
ProtocolName = 'project_MLRraw'; % Enter the name of your protocol
sProtocol.Comment = ProtocolName;
% sProtocol.SUBJECTS = [home 'anat'];
sProtocol.SUBJECTS = '~/brainstorm_db/project_MLRraw/anat';
sProtocol.STUDIES = '~/brainstorm_db/project_MLRraw/data';
...
subject_loop = 1; % FOR STIMULUS ARTEFACT CORRECTION, 
% SET TO 1 IF USING THE LOOP THROUGH ONE SUBJECT AT A TIME

% Depending on whether first step was already launched in a screen
% clear
% parloop()
root_dir = '/private/path/project';
root_dir_bs = '~/brainstorm_db/project_MLRraw'; 
...
stimulus_artefact = {'xxxx', 'xxxx', 'xxxx'; {'A', 'B', 'D'}, {'A', 'B', 'D', 'E', 'F'}, {'A', 'C', 'D', 'G'}};
% First time running these subjects, comment this, we first want to plot
% their individual sessions with the block design, then redo with this
...
level_data = {'sensor', 'source'}; % to distinguish the two types of averages obtained (interpolated and not)
modality_data = {'EEG','MEG','both_mod'};
wave = {'MLR'}; % inherited from previous structure, there is loop with only one value
epoch_wave = {[-0.05, 0.1]}; % MLR only
epoch_wave_bas = {[-0.05, 0.6]}; % MLR, only to create templates for correction of shortest ISI
epoch_baseline = {[-0.05, 0]}; % MLR baseline correction
...
MLR_lowpass = 100;
notch_filter = 1; % 0 = no 60Hz notch filter; 1 = YES
...
source_inverse_measure = 'dspm2018'; % 'amplitude' or 'dspm2018' % ONLY FOR CORTICAL ONES! (NOT MLR)
...
group_default_volume = 'tess_cortex_mixed_02.mat'; % volume surface where all volume sources of invididual subjects will be projected
...
%% Import raw data MLR (EEG/MEG) (import Anatomy with HCP is to be used in local with CNRL_import_anatomy)
% Its going to import files liisted in session_block_array. Double check
% that they are the same 
tic
disp(' ');      
disp('-------------------------');  
disp('IMPORTING EEG/MEG DATA FOR MLR');
disp(datetime)
disp('-------------------------');     
disp(' '); 

Import_error_log_MLR = {};
error_import_MLR = 1;
    ...
    for s = 1:length(session)
        ...
        for b = 1:length(block)
            ...
            for r = 1:length(runs_in_block)    
                results =  endsWith({folders.name},'_AMICA_10Hz.fif') & (...
                    contains({folders.name},[participant{p}(1:4) session{s} '_' runs_in_block{r}]));
                infolder = find(results);
                if isempty(infolder)
                    Import_error_log_MLR{error_import_MLR} = ([participant{p}(1:4) session{s} '_' runs_in_block{r} ': tsss AMICA not found']);
                    error_import_MLR = error_import_MLR +1;
                    continue
                end
                ...
                disp(' ');      
                disp('-------------------------');  
                disp(['Importing data EEG/MEG MLR data for ' participant{p} '_' session{s} '_' block{b} '_' runs_in_block{r}]);
                disp(datetime)
                disp(' '); 
...
save([root_dir '/Events/Import_error_log_MLR.mat'],'Import_error_log_MLR')
clearvars('-except', initialVars{:});

disp 'DONE WITH IMPORTING DATA EEG/MEG for MLR!!!'
disp(datetime)
toc

%% Remove ANY previous active projectors for MLR before analyzing the data
tic
disp(' ');      
disp('-------------------------');  
disp('REMOVING ANY ACTIVE PROJECTORS FOR MLR BEFORE ANYTHING');  
disp(datetime)
disp('-------------------------');     
disp(' '); 
...
for p = 1:length(participant)
    ...
    % Find the raw folders for each participant and session
        ...
        results =  contains({folders.name},'raw') & contains({folders.name},[participant{p}(1:4) session{s}]) & endsWith({folders.name},'tsss_AMICA_10Hz');
        infolder = find(results);
            if isempty(infolder)
                continue
            end
        for l= 1:length(infolder)
            ...
            disp(' ');      
            disp('-------------------------');  
            disp(['Removing active projectors for MLR before anything: ' participant{p} '_' session{s}]);
            disp(datetime)
            disp(' '); 
...
clearvars('-except', initialVars{:});
disp 'DONE WITH REMOVING ANY ACTIVE PROJECTORS FOR MLR BEFORE ANYTHING!!!'
disp(datetime)
toc

%% Import events to raw data MLR
tic
disp(' ');      
disp('-------------------------');  
disp('IMPORTING EVENTS TO RAW DATA FOR MLR');  
disp(datetime)
disp('-------------------------');     
disp(' '); 
...
for p = 1:length(participant)
    ...
    disp(' ');      
    disp('-------------------------');
    disp(['importing events for MLR participant ' participant{p}]);
    disp(datetime)
    disp(' '); 
    ...
    % Find the raw folders for each participant and session
        ...
        disp(' ');      
        disp('-------------------------');  
        disp(['Importing events MLR for ' participant{p} '_' session{s}]);
        disp(datetime)
        disp(' '); 
...
clearvars('-except', initialVars{:});
disp 'DONE WITH IMPORTING EVENTS TO RAW DATA FOR MLR!!!'
disp(datetime)
toc

%% Filtering
tic
% IMPORTANT NOTE: IF COVARIANCE SECTION WAS RUN (there are cov matrices for that subject)
% THEN WE MUST RERUN FILTERING BEFORE RERUN ANY EPOCHING 
% (since in filtered folders -high and low- 91, 92 and 93 
% triggers 3500 ms close to boundaryEnd would have been removed)

disp(' ');      
disp('-------------------------');  
disp('FILTERING FOR MLR');  
disp(datetime)
disp('-------------------------');     
disp(' '); 
...
for p = 1:length(participant)
    ...
    % Find the event folders for each participant and session
        ...
        disp(' ');      
        disp('-------------------------');  
        disp(['Filtering for MLR ' participant{p} '_' session{s}]);
        disp(datetime)
        disp(' ');  
        ...
        % Process: MLR filter (low pass)
        MLR_Files = bst_process('CallProcess', 'process_bandpass', sFiles, [], ...
            'sensortypes', '', ...
            'highpass',    MLR_highpass, ... 
            'lowpass',     MLR_lowpass, ... 
            'tranband',    tranband_MLR, ...
        ...
        % Process: Add tag: MLR
        MLR_Files = bst_process('CallProcess', 'process_add_tag', MLR_Files, [], ...
            'tag',           'MLR', ...
        ...
        infolder = find(results); 
        for l= 1:length(infolder) 
            % We loop again through each folder ending in 'matlab', only this time is the whole path
            line = infolder(l);
...
disp(' ');      
disp('-------------------------');  
disp('OBTAINING ELONGATED MLR EPOCHS FOR BASELINE CORRECTION (Part 1)'); 
disp(datetime)
disp('-------------------------');     
disp(' ');
...
for p = 1:length(participant)
    ...
    for s = 1:length(session)
        ...
        for b = 1:length(block)
            ...
            disp(' ');      
            disp('-------------------------');
            disp(['Renaming MLR events for templates ' participant{p} '_' wave{w} '_' session{s} '_' block{b}]);
            disp(datetime)
            disp(' ');  
            ...
            disp(' ');      
            disp('-------------------------');  
            disp(['Making MLR template epochs for ' participant{p} '_' wave{w} '_' session{s} '_' block{b}]);
            disp(datetime)
            disp(' ');  
            ...
            disp(' ');      
            disp('-------------------------');  
            disp(['Detrend MLR template epochs for ' participant{p} '_' wave{w} '_' session{s} '_' block{b}]);
            disp(datetime)
            disp(' ');  
            ...
            sFiles = bst_process('CallProcess', 'CNRL_BVA_process_detrend_MLR', sFiles, [], ...
            ...
            disp(' ');      
            disp('-------------------------');  
            disp(['Baseline correction for MLR templates ' participant{p} '_' wave{w} '_' session{s} '_' block{b}]);
            disp(datetime)
            disp(' ');
...
clearvars('-except', initialVars{:});
disp 'DONE WITH OBTAINING MLR ELONGATED EPOCHS FOR BASELINE CORRECTION (Part 1)!!!'
disp(datetime)
toc
end
...
disp(' ');      
disp('-------------------------');  
disp('RENAMING FILTERED FILES BACK TO ORIGINAL NAMES (MLR TEMPLATE)');  
disp(datetime)
disp('-------------------------');     
disp(' '); 
    ...
    for s = 1:length(session)
        ...
        for b = 1:length(block)
            ...
            for c =1:length(condition)
                disp(' ');      
                disp('-------------------------');  
                disp(['Renaming template MLR back ' participant{p} '_' wave{w} '_' condition{c} session{s} '_' block{b}]);
                disp(datetime)
                disp(' ');  
...
clearvars('-except', initialVars{:});
disp 'DONE WITH RENAMING BACK EVENTS FROM FILTERED DATAFILES (MLR TEMPLATE)!!!'
disp(datetime)
toc
end
...
for p = 1:length(participant)
    ...
    for s = 1:length(session)
        ...
        for b = 1:length(block)
            ...
            disp(' ');      
            disp('-------------------------');
            disp(['Renaming events for normal epochs MLR ' participant{p} '_' wave{w} '_' session{s} '_' block{b}]);
            disp(datetime)
            disp(' ');  
            ...
            disp(' ');      
            disp('-------------------------');  
            disp(['Making normal MLR epochs (part 1) for ' participant{p} '_' wave{w} '_' session{s} '_' block{b}]);
            disp(datetime)
            disp(' ');  
            ...
            disp(' ');      
            disp('-------------------------');  
            disp(['Detrend normal MLR epochs (part 1) for ' participant{p} '_' wave{w} '_' session{s} '_' block{b}]);
            disp(datetime)
            disp(' ');  
            ...
            disp(' ');      
            disp('-------------------------');  
            disp(['Baseline correcting normal MLR epochs (part 1) for ' participant{p} '_' wave{w} '_' session{s} '_' block{b}]);
            disp(datetime)
            disp(' ');  
    ...
    for s = 1:length(session)
        ...
        for b = 1:length(block)
            ...
            ...%    'datafile',     RawFiles, ...
                ...
                'usectfcomp',   0, ...
                'usessp',       1, ...
                'freq',         [], ... % CHECK IF WE HAVE ENOUGH DATAPOINTS WITH THIS...
                'baseline',     []); 
...
disp(' ');      
disp('-------------------------');  
disp('MOVING MLR EPOCHED FILES TO SEGMENTED PROTOCOL');  
disp(datetime)
disp('-------------------------');     
disp(' '); 

new_root_dir_bs = '~/brainstorm_db/project_MLRseg'; % depends on whether is LLR or MLR

for par = 1:length(participant_general_list)
    
    disp(' ');      
    disp('-------------------------');
    disp(['Moving MLR epoched files to segmented protocol for participant ' participant_general_list{par}]);
    disp(datetime)
    disp(' ');  
    ...
    results = startsWith({folders.name},'MLR'); % ALL EPOCHED START WITH THAT
    infolder = find(results);
...
bst_set('BrainstormDbDir',BrainstormDbDir)
% Select the correct protocol
ProtocolName = 'project_MLRseg'; % Enter the name of your protocol
sProtocol.Comment = ProtocolName;
% sProtocol.SUBJECTS = [home 'anat'];
sProtocol.SUBJECTS = '~/brainstorm_db/project_MLRseg/anat';
sProtocol.STUDIES = '~/brainstorm_db/project_MLRseg/data';
db_edit_protocol('load',sProtocol);
% Get the protocol index
iProtocol = bst_get('Protocol', ProtocolName);
if isempty(iProtocol)
...
disp 'DONE MOVING MLR EPOCHED FILES TO SEGMENTED PROTOCOL!!!'
disp(datetime)
toc
...
disp(' ');      
disp('-------------------------');  
disp('OBTAINING ELONGATED MLR EPOCHS FOR BASELINE CORRECTION (Part 2)'); 
disp(datetime)
disp('-------------------------');     
disp(' ');
    ...
    for s = 1:length(session)
        ...
        for b = 1:length(block)
            for c = 4:length(condition) % ONLY 31 ONWARDS
                ...
                disp(' ');      
                disp('-------------------------');  
                disp(['Cleaning MLR template epochs for ' participant{p} '_' wave{w} '_' condition{c} session{s} '_' block{b} '(EEG)']);
                disp(datetime)
                disp(' '); 
                ...
                disp(' ');      
                disp('-------------------------');  
                disp(['Cleaning MLR template epochs for ' participant{p} '_' wave{w} '_' condition{c} session{s} '_' block{b} '(MEG)']);
                disp(datetime)
                disp(' '); 
                ...
                disp(' ');      
                disp('-------------------------');  
                disp(['Cleaning MLR template epochs for ' participant{p} '_' wave{w} '_' condition{c} session{s} '_' block{b} '(both_mod)']);
                disp(datetime)
                disp(' '); 
                ...
                sFiles_both_mod = bst_process('CallProcess', 'process_CNRL_detectbad',sFiles, [], ...
                    'timewindow', [], ...
                    'meggrad',    reject_MEG_GRAD_absolute, ...
                    'megmag',     reject_MEG_GRAD_absolute, ...
                    'eeg',        reject_EEG_absolute, ...
                    'ieeg',       [0, 0], ...
                    'eog',        [0, 0], ...
                    'ecg',        [0, 0], ...
                ...
                disp(' ');      
                disp('-------------------------');  
                disp(['Averaging MLR template epochs for ' participant{p} '_' wave{w} '_' condition{c} session{s} '_' block{b} '(EEG sensor)']);
                disp(datetime)
                disp(' '); 
                ...
                disp(' ');      
                disp('-------------------------');  
                disp(['Averaging MLR template epochs for ' participant{p} '_' wave{w} '_' condition{c} session{s} '_' block{b} '(MEG sensor)']);
                disp(datetime)
                disp(' '); 
...
%                 'overwrite',   1);
                ...
                disp(' ');      
                disp('-------------------------');  
                disp(['Averaging MLR template epochs for ' participant{p} '_' wave{w} '_' condition{c} session{s} '_' block{b} '(both mod sensor)']);
                disp(datetime)
                disp(' '); 
...
clearvars('-except', initialVars{:});
disp 'DONE WITH OBTAINING MLR ELONGATED EPOCHS FOR BASELINE CORRECTION (Part 2)!!!'
disp(datetime)
toc
...
disp(' ');      
disp('-------------------------');  
disp('AVERAGING MLR TEMPLATES ACROSS BLOCKS (IN CASE BLOCK DATA IS INSUFFICIENT FOR SUBSTRACTION APPROACH)'); 
disp(datetime)
disp('-------------------------');
disp(' ');
...
clearvars('-except', initialVars{:});
disp 'DONE WITH AVERAGING MLR TEMPLATES ACROSS BLOCKS!!!'
disp(datetime)
toc
end
...
disp(' ');      
disp('-------------------------');  
disp('AVERAGING MLR WAVEFORMS ACROSS BLOCKS FROM ALL SESSIONS (TEMPLATE EPOCHS)'); 
disp(datetime)
disp('-------------------------');
disp(' ');
...
for p = 1:length(participant)
    ...
    for w = 1:length(wave)
        for c = 4:length(condition) % ONLY 31 ONWARDS
            ...
            disp(' ');      
            disp('-------------------------');  
            disp(['Averaging MLR blocks from all sessions (template epochs) for ' participant{p} '_' modality_data{mode} '_' wave{w} '_' condition{c}]);
            disp(datetime)
            disp(' ');
...
clearvars('-except', initialVars{:});
disp 'DONE WITH AVERAGING MLR WAVEFORMS ACROSS BLOCKS FROM ALL SESSIONS (TEMPLATE EPOCHS)!!!'
disp(datetime)
toc
...
clearvars('-except', initialVars{:});
disp 'DONE WITH GETTING MLR TEMPLATES OUT OF BRAINSTORM (TO PLOT TEMPLATES)!!!'
disp(datetime)
toc
end
...
%% SUBTRACT from every sweep vectors obtained from templates (MLR)
...
disp(' ');      
disp('-------------------------');  
disp('MLR SUBTRACTION OF ARTEFACT FROM SHORTEST ISI (EFFECTS FROM PREVIOUS TRIAL)');  
disp(datetime)
disp('-------------------------');     
disp(' ');
            ...
            for w = 1:length(wave) % WILL BE LLR ONLY IN THIS CASE
                ...
                infolder = find(results);
                    ...
                    for j= 1:length(trial_count) % for every trial
                        ...
                        eval(['ident_pos = find(abs(trial_time  - Log_' shortest_ISI{si} '_' participant{p} session{s} '_r' trial_run '(:,1))<10^-3);'])
                        ...
                        init_point = prev_lat*1500; % IN THE CASE OF MLR ONLY!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
clearvars('-except', initialVars{:});
disp 'DONE WITH MLR SUBTRACTION OF ARTEFACT FROM SHORTEST ISI (EFFECTS FROM PREVIOUS TRIAL)!!!'
disp(datetime)
toc
end
...
% Import filtered data this time (matlab_low folders for MLR)
for p = 1:length(participant)
...
%% Count trials for NORMAL epochs FOR MLR
...
disp(' ');      
disp('-------------------------');  
disp('COUNTING TRIALS FOR NORMAL EPOCHS MLR'); 
disp(datetime)
disp('-------------------------');     
disp(' ');

if sensor_analysis == 1
    colNames = {'PRE_MLR','EEG_Post'};
    modality_data = {'EEG'};
elseif sensor_analysis == 2
    colNames = {'PRE_MLR','MEG_Post'};
    modality_data = {'MEG'};
elseif sensor_analysis == 3
    colNames = {'PRE_MLR','Combined_Post'};
    modality_data = {'both_mod'};
elseif sensor_analysis == 4
    colNames = {'PRE_MLR','EEG_Post','MEG_Post','Combined_Post'};
end
...
for p = 1:length(participant)
    ...
    for w = 1:length(modality_data) % Just because of inheritance from previous script, we keep 'w' instead of 'mode'
        ...
        for s = 1:length(session) 
            ...
            for b = 1:length(block)
                for c =1:length(condition)
                    folders = dir([root_dir_bs '/data/' participant{p} '/']);
                    results = contains({folders.name},['MLR_' condition{c} session{s} '_' block{b} '_normal']);
                    infolder = find(results);
                    if isempty(infolder)
                        continue
                        % error(['No MLR_' condition{c} session{s} '_' block{b} '_normal for participant ' participant{p}])
                    end  
                    
                    disp(' ');      
                    disp('-------------------------');
                    disp(['Counting MLR normal sweeps for ' participant{p} '_MLR_' modality_data{w} '_' session{s} '_' block{b} '_' condition{c}]);
                    disp(' '); 
    ...
    eval(['MLR_' participant{p} '_Normal_trials_detailed = array2table(Normal_trials_detailed,''RowNames'',rowNames_detailed,''VariableNames'',colNames);'])
    eval(['MLR_' participant{p} '_Normal_trials_summary = array2table(Normal_trials_summary,''RowNames'',rowNames_summary,''VariableNames'',colNames);'])
    save([root_dir '/Events/MLR_Counts_' participant{p} '_Normal_detailed.mat'],['MLR_' participant{p} '_Normal_trials_detailed']);
    save([root_dir '/Events/MLR_Counts_' participant{p} '_Normal_summary.mat'],['MLR_' participant{p} '_Normal_trials_summary']);
end
...
clearvars('-except', initialVars{:});
disp 'DONE WITH COUNTING TRIALS FOR NORMAL EPOCHS FOR MLR!!!'
disp(datetime)
toc

%% Stimulus artefact correction (when necessary)
% Before further averaging, correct first if indicated in Define variables
% (i.e. stimulus_artefact is not empty)

if sensor_analysis ~= 1 % ONLY RUN THIS PART IF SENSOR DATA IS NOT EEG ONLY (1)
tic

if sensor_analysis == 2
    modality_data = {'MEG'};
    init_iter_mode = 1;
elseif sensor_analysis == 3
    modality_data = {'both_mod'};
...
if subject_loop == 1
    % THIS IS ONLY IF WE ARE LOOPING ONE SUBJECT AT A TIME THROUGH ALL
    results = contains(stimulus_artefact(1,:),[participant{1}]);
    infolder = find(results);
    if ~isempty(infolder)
        stimulus_artefact = stimulus_artefact(:,infolder);
    end
end

disp(' ');      
disp('-------------------------');  
disp('CORRECTING STIMULUS ARTEFACT'); 
disp(datetime)
disp('-------------------------');
disp(' ');

for p= 1:size(stimulus_artefact,2) % for every subject needing correction
    
    disp(' ');      
    disp('-------------------------');
    disp(['loading participant ' stimulus_artefact{1,p}]);
    disp(datetime)
    disp(' '); 
        
    prot_subs = bst_get('ProtocolSubjects');
    current_sub = find(strcmp({prot_subs.Subject.Name}, stimulus_artefact{1,p}));
    db_reload_conditions(current_sub);
    
    for s=1:length(stimulus_artefact{2,p}) % number of sessions needing correction for that subject
        sess_artifact = stimulus_artefact{2,p}; % actual names of sessions for that subject
        pos_par = find(strcmp(session_block_array(:,1), stimulus_artefact{1,p}(1:4)));
        pos_ses = find(strcmp(session_block_array{pos_par,2}(1,:), sess_artifact{s}));
        block = session_block_array{pos_par,2}{2,pos_ses}(1,:);
        
        for b = 1:length(block)
            for c = 1:length(loudest_condition) % only loudest ones
                for mode = init_iter_mode:length(modality_data) %%%%%%%%% ONLY 2 AND 3!!
                    folders = dir([root_dir_bs '/data/' stimulus_artefact{1,p} '/']);
                    % Only in MLR
                    results =  contains({folders.name},['MLR_' loudest_condition{c} sess_artifact{s} '_' block{b} '_normal']);
                    infolder = find(results);
                    if isempty(infolder)
                        continue
                    end
                    sub_folders = dir([root_dir_bs '/data/' stimulus_artefact{1,p} '/' folders(infolder).name '/']);
                    for lvl = 1:length(level_data)
                        sub_results =  contains({sub_folders.name},[modality_data{mode} '_average_' level_data{lvl}]);
                        sub_infolder = find(sub_results);
                        if isempty(sub_infolder) % if average is not present despite th folder existing
                            continue
                        end
                        file_name = [stimulus_artefact{1,p} '/' folders(infolder).name '/' sub_folders(sub_infolder).name]; % sub_results (using logicals)
                        ...
                        disp(' ');      
                        disp('-------------------------');  
                        disp(['Correcting stimulus artefact for ' stimulus_artefact{1,p} '_MLR_' loudest_condition{c} sess_artifact{s} '_' block{b}]);
                        disp(datetime)
                        disp(' ');

                        % Process: Cut stimulation artifact: [-5ms,5ms]
                        sFiles = bst_process('CallProcess', 'process_cutstim', sFiles, [], ...
                        'eventname',   '', ...
                        'timewindow',  [-0.0053, 0.0053], ... % NO DOWNSAMPLING FOR MLR, SO THE SAME
                        'sensortypes', 'MEG', ... % ONLY NECESSARY FOR MEG
                        'method',      'linear', ...  % linear
                        'overwrite',   1);
                        % IT WILL OVERWRITE THE AVERAGE FILE AND ADD 'cutstim' tag
                    end
                end
            end
        end
...
% reset stimulus artefact to original if looping once every subject
if subject_loop == 1
...
clearvars('-except', initialVars{:});
disp 'DONE WITH CORRECTING STIMULUS ARTEFACT!!!'
disp(datetime)
toc
...
%% Collapse SENSOR DATA across ALL CONDITIONS (to have representation of MLR)
tic
disp(' ');      
disp('-------------------------');  
...
disp(' ');      
disp('-------------------------');  
disp('COUNTING TRIALS FOR COVARIANCE EPOCHS (MLR)'); 
disp(datetime)
disp('-------------------------');     
disp(' ');
...
for p = 1:length(participant)
    ...
    for w = 1:length(modality_data) % Just because of inheritance from previous script, we keep 'w' instead of 'mode' 
        ...
        for s = 1:length(session)
            ...
            for b = 1:length(block) 
                folders = dir([root_dir_bs '/data/' participant{p} '/']);
                results = contains({folders.name},['MLR_91' session{s} '_' block{b} '_covariance'])...
                    | contains({folders.name},['MLR_92' session{s} '_' block{b} '_covariance'])...
                    | contains({folders.name},['MLR_93' session{s} '_' block{b} '_covariance']);
                infolder = find(results);
                if isempty(infolder)
                    continue
                end
                ...
                disp(' ');      
                disp('-------------------------');
                disp(['Counting covariance sweeps for ' participant{p} 'MLR_' modality_data{w} '_' session{s} '_' block{b}]);
                disp(' '); 
    ...
    % Create tables from data matrix and save
    eval(['MLR_' participant{p} '_Cov_trials = array2table(Cov_trials,''RowNames'',rowNames_detailed,''VariableNames'',colNames);'])
    save([root_dir '/Events/MLR_Counts_' participant{p} '_Covariance.mat'],['MLR_' participant{p} '_Cov_trials']);
end
...
clearvars('-except', initialVars{:});
disp 'DONE WITH COUNTING TRIALS FOR COVARIANCE EPOCHS (MLR)!!!'
disp(datetime)
toc
...
clearvars('-except', initialVars{:});
disp 'DONE WITH GAVR SENSOR DATA (MLR)!!!'
disp(datetime)
toc
...
%% Copy channel files with corregistration from LLR protocol
% Take into account that LLR 2320 has one session with no trggers in LLR
% but these are present in MLR (but no corregistration for that one)

% CHANNEL FILES FROM LLR SEG FOLDERS SHOULD BE COPIED OVER HERE. ALL THE
% TRANSFORMATION SHOULD BE THE SAME BETWEEN LLR AND MLR if the session and
% block are the same. Any discrepancies are informed

tic
disp(' ');      
disp('-------------------------');  
disp('COPYING TRANSFORMATION MATRICES FROM LLR TO MLR');  
disp(datetime)
disp('-------------------------');     
disp(' '); 

root_dir_MLRseg = '~/brainstorm_db/project_MLRseg';
root_dir_LLRseg = '~/brainstorm_db/project_LLRseg';
...
for p = 1:length(participant)
    ...
    for s = 1:length(session)
        ...
        for b = 1:length(block)
            for c =1:length(condition)
                % If that channel file exist in MLR seg
                if exist([root_dir_MLRseg '/data/' participant{p} '/MLR_' condition{c} session{s} '_' block{b} '_normal/channel_vectorview306_acc1.mat'],'file')
                    % AND that very same channel file exist in LLR seg
                    if exist([root_dir_LLRseg '/data/' participant{p} '/LLR_' condition{c} session{s} '_' block{b} '_normal/channel_vectorview306_acc1.mat'],'file')
                        channel_file_MLR = [root_dir_MLRseg '/data/' participant{p} '/MLR_' condition{c} session{s} '_' block{b} '_normal/channel_vectorview306_acc1.mat'];
                        channel_file_LLR = [root_dir_LLRseg '/data/' participant{p} '/LLR_' condition{c} session{s} '_' block{b} '_normal/channel_vectorview306_acc1.mat'];
                        % delete original MLR channel file
                        delete(channel_file_MLR);
                        % copy LLR channel file into equivalent MLR folder
                        copyfile(channel_file_LLR,channel_file_MLR);
                        % it will contain all transformations
                    else
                        % MLR exists but not equivalent LLR to copy
                        channel_file_copy_log{log_channel,1} = ['MLR but not LLR in ' condition{c} session{s} '_' block{b} '_normal'];
                        log_channel = log_channel + 1; 
                    end
                else
                    if exist([root_dir_LLRseg '/data/' participant{p} '/LLR_' condition{c} session{s} '_' block{b} '_normal/channel_vectorview306_acc1.mat'],'file')
                        channel_file_copy_log{log_channel,1} = ['LLR but not MLR in ' condition{c} session{s} '_' block{b} '_normal'];
                        log_channel = log_channel + 1; 
                    else
                        channel_file_copy_log{log_channel,1} = ['not MLR or LLR in ' condition{c} session{s} '_' block{b} '_normal'];
                        log_channel = log_channel + 1; 
                    end
                end
            end
...
%% Copy the bad channels selected from LLR new protocol to MLR old protocol before EEG sources

tic
disp(' ');      
disp('-------------------------');  
disp('COPYING BAD CHANNELS FROM HUMON_LLR TO MLRseg before EEG sources');  
disp(datetime)
disp('-------------------------');     
disp(' '); 
...
root_dir_Destiny = [root_dir '/brainstorm_db/MLRseg']; 
...
for p = 1:length(participant)
    ...
    for s = 1:length(session)
        ...
        for c = 1:length(condition)
            ...
            results = contains({folders.name},['MLR_' condition{c} session{s} '_' block{b} '_normal']) | ...
                contains({folders.name},['MLR_' condition{c} session{s} '_' block{b} '_covariance']);
            infolder = find(results);
...
disp 'DONE WITH COPYING BAD CHANNELS FROM HUMON_LLR TO MLRseg!!!'
disp(datetime)
toc
...
disp(' ');      
disp('-------------------------');  
disp('CALCULATING NOISE COVARIANCE (MLR)'); 
disp(datetime)
disp('-------------------------');     
disp(' ');
...
% Compute matrices
for p = 1:length(participant)
    eval(['Insuff_cov_trials_MLR_' participant{p} '= {};'])
    count_insuff_cov = 1;
            ...
            for w = 1:length(wave)
                for mode = 1:length(modality_data) % EEG, MEG and both mod    
                    ...
                    if source_analysis == 1 % EEG
                        if size(sFiles,2) < 1
                            eval(['Insuff_cov_trials_MLR_' participant{p} '{count_insuff_cov,1} = [wave{w} ''_'' session{s} ''_'' modality_data{mode} ''_'' block{b}];'])
    ...
    save([root_dir '/Sources/Insuff_cov_trials_MLR_' participant{p} '.mat'],['Insuff_cov_trials_MLR_' participant{p}]);
end
...
clearvars('-except', initialVars{:});
disp 'DONE WITH CALCULATING NOISE COVARIANCE (MLR)!!!'
disp(datetime)
toc

end

%% Copy forward model with surface from LLR (combined EEG + MEG one)

if any(contains(source_space,'Surf'))

tic
disp(' ');      
disp('-------------------------');  
disp('COPYING CORTICAL SURFACE FORWARD MODEL FROM LLR TO MLR');  
disp(datetime)
disp('-------------------------');     
disp(' '); 
...
for p = 1:length(participant)
    ...
    for s = 1:length(session)
        ...
        for b = 1:length(block)
            for c =1:length(condition)
                % If that channel file exist in MLR seg
                % headmodel_vol_duneuro_os_meg.mat
                % headmodel_surf_duneuro_os_meg.mat
                if exist([root_dir_MLRseg '/data/' participant{p} '/MLR_' condition{c} session{s} '_' block{b} '_normal/'],'dir')
                    % AND that very same channel file exist in LLR seg
                    if exist([root_dir_LLRseg '/data/' participant{p} '/LLR_' condition{c} session{s} '_' block{b} '_normal/headmodel_surf_duneuro_os_meg.mat'],'file')
                        forward_file_MLR = [root_dir_MLRseg '/data/' participant{p} '/MLR_' condition{c} session{s} '_' block{b} '_normal/headmodel_surf_duneuro_os_meg.mat'];
                        forward_file_LLR = [root_dir_LLRseg '/data/' participant{p} '/LLR_' condition{c} session{s} '_' block{b} '_normal/headmodel_surf_duneuro_os_meg.mat'];
                        % if existing, delete original MLR surface forward model file
                        if exist(forward_file_MLR,'file')
                            delete(forward_file_MLR);
                        end
                        % copy LLR surface forward model file into equivalent MLR folder
                        copyfile(forward_file_LLR,forward_file_MLR);
                        % it will contain all transformations
                    else
                        % MLR exists but not equivalent LLR to copy
                        forward_file_copy_log{log_forward,1} = ['MLR folder but not LLR file in ' condition{c} session{s} '_' block{b} '_normal'];
                        log_forward = log_forward + 1; 
                    end
                else
                    if exist([root_dir_LLRseg '/data/' participant{p} '/LLR_' condition{c} session{s} '_' block{b} '_normal/headmodel_surf_duneuro_os_meg.mat'],'file')
                        forward_file_copy_log{log_forward,1} = ['LLR file but not MLR folder in ' condition{c} session{s} '_' block{b} '_normal'];
                        log_forward = log_forward + 1; 
                    else
                        forward_file_copy_log{log_forward,1} = ['not MLR folder or LLR file in ' condition{c} session{s} '_' block{b} '_normal'];
                        log_forward = log_forward + 1; 
                    end
                end
            end
...
save([root_dir '/Events/forward_file_copy_log.mat'],'forward_file_copy_log');

clearvars('-except', initialVars{:});
disp 'DONE WITH COPYING CORTICAL SURFACE FORWARD MODEL FROM LLR TO MLR!!!'
disp(datetime)
toc

end

%% HERE YOU HAVE TO DECIDE WHETHER CALCULATING CORTICAL OR VOLUME FORWARD MODEL AND INVERSE SOL

% IF AIMING FOR THE VOLUME:
% FIRST, SELECT IN DEFAULT ANATOMY THE BRAIN WITH THE CEREBELUM AS DEFAULT (GREEN). 
% THEN, GO TO FUNCTIONAL TAB, GROUP ANALYSIS/COMMON FILES AND CREATE A GRID: DONE (February 2021)
% THAT GRID SHOULD BE CALLED IN CASE OF VOLUME ONE

% AS FOR INVERSE SOLUTIONS, THE ONLY DIFFERENCE IS LOOSE CONSTRAINS FOR
% CORTEX AND FREE FOR VOLUME

%% Calculate VOLUME forward model (HAS TO BE IN 98) CORTICAL ONE IS THE SAME THAN LLR
% Compute leadfields only once per each participant, session and block

if any(contains(source_space,'Vol'))

tic
disp(' ');      
disp('-------------------------');
disp('COMPUTING FORWARD VOLUME MODEL (EEG and MEG) FOR EACH PARTICIPANT, SESSION AND BLOCK (MLR)');
disp(datetime)
disp('-------------------------');
disp(' ');
...
for p = 1:length(participant)
    ...
    for s = 1:length(session)
        ...
        for b = 1:length(block)
            ...
            % We could chose any other, but let's take the MLR_11
            if ~exist([root_dir_bs '/data/' participant{p} '/MLR_11' session{s} '_' block{b} '_normal'], 'dir') 
                continue
            end

            folders = dir([root_dir_bs '/data/' participant{p} '/MLR_11' session{s} '_' block{b} '_normal']);            
            % Before it had EEG or MEG in the second contains, but that's
            % not needed, any file will have the same EEG and MEG sensor
            % positions
            results =  contains({folders.name},['MLR_11' session{s} '_' block{b} '_normal']) & contains({folders.name}, '_average_source');
            infolder = find(results);
            if isempty(infolder)
                % In case folder exists but does not contain anything (channel vector or average)
                No_head_model_created{count_no_cov,1} = [participant{p} '_MLR_11' session{s} '_' block{b} '_normal_source'];
            ...
            file_name = [participant{p} '/MLR_11' session{s} '_' block{b} '_normal/' folders(infolder).name];
            ...
            disp(' ');
            disp('-------------------------');  
            disp(['Computing forward VOLUME model (Overlapping spheres & Cortical surface) for ' participant{p} '_' session{s} '_' block{b}]);
            disp(datetime)
            disp(' ');
            ...
            if delete_previous_file == 1
                ...
                if isfile([root_dir_bs '/data/' participant{p} '/MLR_11' session{s} '_' block{b} '_normal/headmodel_vol_os_meg.mat'])
                    delete([root_dir_bs '/data/' participant{p} '/MLR_11' session{s} '_' block{b} '_normal/headmodel_vol_os_meg.mat'])
                end
                % head model = headmodel_vol_os_meg.mat
            ...
            if delete_previous_file == 1
                % check if there is already a headmodel OF THIS TYPE in the folder
                if isfile([root_dir_bs '/data/' participant{p} '/MLR_11' session{s} '_' block{b} '_normal/headmodel_vol_duneuro.mat'])
                    delete([root_dir_bs '/data/' participant{p} '/MLR_11' session{s} '_' block{b} '_normal/headmodel_vol_duneuro.mat'])
                end
                % head model = headmodel_vol_os_meg.mat
                % source = contains KERNEL endswith _Source_Regul_MEG.mat                 
            end
...
for p = 1:length(participant)
    ...
    for s = 1:length(session)
        ...
        % MLR_11 first block is where EEG forward model is
        % If folder does not exists or does not contain the head model, don't even try
        if ~exist([root_dir_bs '/data/' participant{p} '/MLR_11' session{s} '_' block{1} '_normal'], 'dir');continue;end
        if ~exist([root_dir_bs '/data/' participant{p} '/MLR_11' session{s} '_' block{1} '_normal/headmodel_vol_duneuro.mat'],'file')
            error([' no EEG forward model in ' participant{p} '_MLR_11' session{s} '_' block{1} '_normal']);
        end


        disp(' ');      
        disp('-------------------------');  
        disp(['Copying EEG VOLUME forward model from first block to others for ' participant{p} '_' session{s}]);
        disp(datetime)
        disp(' ');

        % The EEG forward model from first block
        file_name_leadfield = [root_dir_bs '/data/' participant{p} '/MLR_11' session{s} '_' block{1} '_normal/headmodel_vol_duneuro.mat'];
        ...
        for b = 2:length(block)
            ...
            if exist([root_dir_bs '/data/' participant{p} '/MLR_11' session{s} '_' block{b} '_normal/'],'dir')
                copyfile(file_name_leadfield,[root_dir_bs '/data/' participant{p} '/MLR_11' session{s} '_' block{b} '_normal/headmodel_vol_duneuro.mat']);
            end
        end
    end
end
...
for p = 1:length(participant)
    ...
    for s = 1:length(session)
        ...
        for b = 1:length(block)
            ...
            if ~exist([root_dir_bs '/data/' participant{p} '/MLR_11' session{s} '_' block{b} '_normal'], 'dir'); continue; end
            if ~exist([root_dir_bs '/data/' participant{p} '/MLR_11' session{s} '_' block{b} '_normal/headmodel_vol_duneuro.mat'],'file'); continue; end
            if ~exist([root_dir_bs '/data/' participant{p} '/MLR_11' session{s} '_' block{b} '_normal/headmodel_vol_os_meg.mat'],'file'); continue; end

            EEG_leadfield = [root_dir_bs '/data/' participant{p} '/MLR_11' session{s} '_' block{b} '_normal/headmodel_vol_duneuro.mat'];
            MEG_leadfield = [root_dir_bs '/data/' participant{p} '/MLR_11' session{s} '_' block{b} '_normal/headmodel_vol_os_meg.mat'];
    
            disp(' ');
            disp('-------------------------');  
            disp(['Merging EEG/MEG VOLUME leadfields for ' participant{p} '_' session{s} '_' block{b}]);
            disp(datetime)
            disp(' ');   
            
            % If stated, find and delete any previous combined leadfield 
            if delete_previous_file == 1
                if isfile([root_dir_bs '/data/' participant{p} '/MLR_11' session{s} '_' block{b} '_normal/headmodel_vol_duneuro_os_meg.mat'])
                    delete([root_dir_bs '/data/' participant{p} '/MLR_11' session{s} '_' block{b} '_normal/headmodel_vol_duneuro_os_meg.mat'])
                end              
            end
            ...
            % computed manually: headmodel_vol_duneuro_os_meg.mat
...
for p = 1:length(participant)
    ...
    for s = 1:length(session)
        ...
        for b = 1:length(block)
            ...
            % MLR_11 is the folder in which they were created in the step before
            if ~exist([root_dir_bs '/data/' participant{p} '/MLR_11' session{s} '_' block{b} '_normal'], 'dir'); continue;end
           
            if ~exist([root_dir_bs '/data/' participant{p} '/MLR_11' session{s} '_' block{b} '_normal/headmodel_vol_duneuro_os_meg.mat'],'file') 
                error(['no combined forward model for ' participant{p} '_MLR_11' session{s} '_' block{b} '_normal'])
            end
            % If folder exists but does not contain the combined head model, don't even try
            
            disp(' ');      
            disp('-------------------------');  
            disp(['Copying between conditions forward VOLUME model (EEG + MEG) for ' participant{p} '_' session{s} '_' block{b}]);
            disp(datetime)
            disp(' ');

            % This name is the one we obtain for the MEG overlapping spheres
            file_name_leadfield = [root_dir_bs '/data/' participant{p} '/MLR_11' session{s} '_' block{b} '_normal/headmodel_vol_duneuro_os_meg.mat'];
            
            for c = 2:length(condition)
                if  strcmp(condition{c}, '11') % IF ITS MLR_11 do not copy, just in case
                    continue
                end
                ...
                if delete_previous_file == 1
                    % check if there is already a headmodel OF THIS
                    % TYPE in the folder where we are going to copy the file
                    if isfile([root_dir_bs '/data/' participant{p} '/MLR_' condition{c} session{s} '_' block{b} '_normal/headmodel_vol_duneuro_os_meg.mat'])
                        delete([root_dir_bs '/data/' participant{p} '/MLR_' condition{c} session{s} '_' block{b} '_normal/headmodel_vol_duneuro_os_meg.mat'])
                    end
                    % head model = headmodel_vol_os_meg.mat
                    % source = contains KERNEL endswith _Source_Regul_MEG.mat                 
                end
                if exist([root_dir_bs '/data/' participant{p} '/MLR_' condition{c} session{s} '_' block{b} '_normal/'],'dir')
                    copyfile(file_name_leadfield,[root_dir_bs '/data/' participant{p} '/MLR_' condition{c} session{s} '_' block{b} '_normal/headmodel_vol_duneuro_os_meg.mat']);
                end
            end
        end
    end
...
clearvars('-except', initialVars{:});
disp 'DONE WITH COMPUTING FORWARD VOLUME MODEL FOR EACH PARTICIPANT, SESSION AND BLOCK (MLR)!!!'
disp(datetime)
toc

end

%% Compute inverse solutions CORTICAL 
...
disp(' ');      
disp('-------------------------');
disp('COMPUTING CORTICAL INVERSE SOLUTIONS (MLR)');
disp(datetime)
disp('-------------------------');
disp(' ');
            ...
            for w = 1:length(wave) %  A WAY TO CHANGE IT EASILY TO MLR
                for mode = 1:length(modality_data)
                    for c = 1:length(condition)
                        ...
                        % TAKE INTO ACCOUNT
                        % 1) We discussed with Brian at one point that
                        ...
                        if strcmp(modality_data{mode},'EEG') % if EEG source solution alone was requested
                            ...
                            disp(' ');
                            disp('-------------------------');  
                            disp(['Computing inverse solutions CORTICAL (EEG) for ' participant{p} '_' wave{w} '_' condition{c} session{s} '_' block{b}]);
                            disp(datetime)
                            disp(' ');
                            ...
                            for nopt = 1:length(source_noise_options)
                                ...
                                if delete_previous_file == 1
                                    ...
                                    results_delete =  contains({folders_delete.name},'KERNEL') & endsWith({folders_delete.name}, ['Surf_' source_noise_tags{nopt} '_' modality_data{mode} '.mat']); 
                                    infolder_delete = find(results_delete);
                                    if ~isempty(infolder_delete) % file exists, therefore delete it
                                ...
                                % NOW WE NEED TO SPECIFY WHICH FORWARD
                                % MODEL IS TH ONE SELECTED, IN THIS CASE,
                                % THE CORTICAL (SURFACE) ONE
                                file_to_search = [participant{p} '/' wave{w} '_' condition{c} session{s} '_' block{b} '_normal/channel_vectorview306_acc1.mat'];
                                [sStudy, iStudy] = bst_get('AnyFile', file_to_search);
                                surface_index = find(contains({sStudy.HeadModel.HeadModelType},'surface'));
                                ...
                                sFiles_source = bst_process('CallProcess', 'process_inverse_2018', sFiles, [], ...
                                    ...
                                    'inverse', struct(...
                                        ...
                                        sFiles_name = bst_process('CallProcess', 'process_add_tag', sFiles_source, [], ...
                                            'tag',           ['Surf_' source_noise_tags{nopt} '_' modality_data{mode}], ...
                        ...
                        if strcmp(modality_data{mode}, 'MEG')
                            ...
                            disp(' ');
                            disp('-------------------------');  
                            disp(['Computing inverse solutions CORTICAL (MEG) for ' participant{p} '_' wave{w} '_' condition{c} session{s} '_' block{b}]);
                            disp(datetime)
                            disp(' ');
                        ...
                        if strcmp(modality_data{mode}, 'both_mod')
                            ...
                            disp(' ');
                            disp('-------------------------');  
                            disp(['Computing inverse solutions CORTICAL (bimodal) for ' participant{p} '_' wave{w} '_' condition{c} session{s} '_' block{b}]);
                            disp(datetime)
                            disp(' ');
...
clearvars('-except', initialVars{:});
disp 'DONE COMPUTING CORTICAL INVERSE SOLUTIONS (MLR)!!!'
disp(datetime)
toc

end

%% Compute inverse solutions VOLUME 
% Separately for every condition, wave, session block and participant
...
disp(' ');      
disp('-------------------------');
disp('COMPUTING VOLUME INVERSE SOLUTIONS (MLR)');
disp(datetime)
disp('-------------------------');
disp(' ');
                ...
                for mode = 1:length(modality_data)
                    for c = 1:length(condition)
                        ...
                        if strcmp(modality_data{mode},'EEG') % if EEG source solution alone was requested
                            ...
                            disp(' ');
                            disp('-------------------------');  
                            disp(['Computing inverse solutions VOLUME (EEG) for ' participant{p} '_' wave{w} '_' condition{c} session{s} '_' block{b}]);
                            disp(datetime)
                            disp(' ');
                            ...
                            for nopt = 1:length(source_noise_options)
                                ...
                                if delete_previous_file == 1
                                    ...
                                    % TYPE in the folder where we are gonna calculate it
                                    folders_delete = dir([root_dir_bs '/data/' participant{p} '/' wave{w} '_' condition{c} session{s} '_' block{b} '_normal']);
                                    results_delete =  contains({folders_delete.name},'KERNEL') & endsWith({folders_delete.name}, ['Vol_' source_noise_tags{nopt} '_' modality_data{mode} '.mat']); 
                                    infolder_delete = find(results_delete);
                                    if ~isempty(infolder_delete) % file exists, therefore delete it
                                       delete([root_dir_bs '/data/' participant{p} '/' wave{w} '_' condition{c} session{s} '_' block{b} '_normal/' folders_delete(infolder_delete).name]);
                                    end
                                ...
                                file_to_search = [participant{p} '/' wave{w} '_' condition{c} session{s} '_' block{b} '_normal/channel_vectorview306_acc1.mat'];
                                [sStudy, iStudy] = bst_get('AnyFile', file_to_search);
                                volume_index = find(contains({sStudy.HeadModel.HeadModelType},'volume'));
                                if length(volume_index) > 1
                                    error('more than one volume head model found');
                                end
                                sStudy.iHeadModel = volume_index;
                                bst_set('Study', iStudy, sStudy);
                                % According to brainstorm developers, better not to reload
                                % db_reload_studies(iStudy);
                                ...
                                sFiles_source = bst_process('CallProcess', 'process_inverse_2018', sFiles, [], ...
                                    ...
                                    'inverse', struct(...
                                        ...
                                        % Process: Change name in folder (for code)
                                        sFiles_name = bst_process('CallProcess', 'process_add_tag', sFiles_source, [], ...
                                            'tag',           ['Vol_' source_noise_tags{nopt} '_' modality_data{mode}], ...
                                            'output',        2);  % Add to file path
                        ...
                        if strcmp(modality_data{mode}, 'MEG')
                            ...
                            disp(' ');
                            disp('-------------------------');  
                            disp(['Computing inverse solutions VOLUME (MEG) for ' participant{p} '_' wave{w} '_' condition{c} session{s} '_' block{b}]);
                            disp(datetime)
                            disp(' ');
...
save([root_dir '/Sources/No_inverse_solution_created.mat'],'No_inverse_solution_created');
clearvars('-except', initialVars{:});
disp 'DONE COMPUTING VOLUME INVERSE SOLUTIONS (MLR)!!!'
disp(datetime)
toc
...
disp(' ');
disp('-------------------------');  
disp('AVERAGING INVERSE SOLUTIONS ACROSS BLOCKS FROM ALL SESSIONS (MLR)'); 
disp(datetime)
disp('-------------------------');
disp(' ');
            ...
            for s = 1:length(session)
                ...
                if isempty(infolder)
                    ...
                    for l = 1:length(infolder) % for each block found
                        ...
                        % End checking if block exist in list
                        sub_folders = dir([root_dir_bs '/data/' participant{p} '/' folders(line).name '/']);
                        sub_results_kernel =  contains({sub_folders.name},[source_space{se} '_' source_noise_tags{nopt} '_' modality_data{mode}]);
            ...
            if ~isempty(sFiles) % in case 'continue' option was selected for all
               ...
               % If stated, find and delete any previous source in intra folder (overwrite it)
                if delete_previous_file == 1
                    ...
                    % TYPE in the folder where we are gonna calculate it
                    folders_delete = dir([root_dir_bs '/data/' participant{p} '/@intra/']);
                    results_delete =  endsWith({folders_delete.name}, [modality_data{mode} '_' source_space{se} '_' source_noise_tags{nopt} '_' wave{w} '_' condition{c} '.mat']); 
                    infolder_delete = find(results_delete);
                    if ~isempty(infolder_delete) % file exists, therefore delete it
                       delete([root_dir_bs '/data/' participant{p} '/@intra/' folders_delete(infolder_delete).name]);
                    end
                ...
                disp(' ');
                disp('-------------------------');  
                disp(['Average ' source_space{se} ' blocks from all sessions (' modality_data{mode} '_' source_noise_tags{nopt} ') for ' participant{p} '_' wave{w} '_' condition{c}]);
                disp(datetime)
                disp(' ');
                ...
                % Process: Add tag:
                sFiles = bst_process('CallProcess', 'process_add_tag', sFiles, [], ...
                    'tag',           [modality_data{mode} '_' source_space{se} '_' source_noise_tags{nopt} '_' wave{w} '_' condition{c}], ...
...
clearvars('-except', initialVars{:});
disp 'DONE AVERAGING INVERSE SOLUTIONS ACROSS BLOCKS FROM ALL SESSIONS (MLR)!!!'
disp(datetime)
toc
...
disp(' ');      
disp('-------------------------');  
disp('AVERAGING INVERSE SOLUTIONS ACROSS BLOCKS (MLR)'); 
disp(datetime)
disp('-------------------------');
disp(' ');
        ...
        for mode = 1:length(modality_data)
            for c = 1:length(condition)
                ...
                for nopt = 1:length(source_noise_tags)
                    ...
                    disp(' ');
                    disp('-------------------------');  
                    disp(['Averaging ' source_space{se} ' sources (' source_noise_tags{nopt} ') across blocks for ' participant{p} '_' modality_data{mode} '_' wave{w} '_' condition{c} session{s}]);
                    disp(datetime)
                    disp(' ');
                    ...
                    if delete_previous_file == 1
                        ...
                        % TYPE in the folder where we are gonna calculate it
                        folders_delete = dir([root_dir_bs '/data/' participant{p} '/@intra/']);
                        results_delete =  endsWith({folders_delete.name}, [modality_data{mode} '_' source_space{se} '_' source_noise_tags{nopt} '_' wave{w} '_' condition{c} session{s} '.mat']); 
                        infolder_delete = find(results_delete);
                        if ~isempty(infolder_delete) % file exists, therefore delete it
                           delete([root_dir_bs '/data/' participant{p} '/@intra/' folders_delete(infolder_delete).name]);
                        end
                    ...
                    % Process: Add tag:
                    sFiles = bst_process('CallProcess', 'process_add_tag', sFiles, [], ...
                        'tag',           [modality_data{mode} '_' source_space{se} '_' source_noise_tags{nopt} '_' wave{w} '_' condition{c} session{s}], ...
                        'output',        2);  % Add to file name
...
clearvars('-except', initialVars{:});
disp 'DONE WITH AVERAGING INVERSE SOLUTIONS ACROSS BLOCKS (MLR)!!!'
disp(datetime)
toc
        ...
        for se = 1:length(source_space)  
            for s = 1:length(session)
                ...
                for nopt = 1:length(source_noise_tags)
                    ...
                    disp(' ');
                    disp('-------------------------');  
                    disp(['Average ' source_space{se} ' sources (' source_noise_tags{nopt} ') across all blocks and conditions for ' participant{p} '_' modality_data{mode} '_' wave{w} '_' session{s}]);
                    disp(datetime)
                    disp(' ');
                    ...
                    if delete_previous_file == 1
                        ...
                        % TYPE in the folder where we are gonna calculate it
                        folders_delete = dir([root_dir_bs '/data/' participant{p} '/@intra/']);
                        results_delete = endsWith({folders_delete.name}, [modality_data{mode} '_' source_space{se} '_' source_noise_tags{nopt} '_' wave{w} '_ALL_' session{s} '.mat']); 
                        infolder_delete = find(results_delete);
                        if ~isempty(infolder_delete) % file exists, therefore delete it
                           delete([root_dir_bs '/data/' participant{p} '/@intra/' folders_delete(infolder_delete).name]);
                        end
                    ...
                    % Process: Add tag:
                    sFiles = bst_process('CallProcess', 'process_add_tag', sFiles, [], ...
                        'tag',           [modality_data{mode} '_' source_space{se} '_' source_noise_tags{nopt} '_' wave{w} '_ALL_' session{s}], ...
                        'output',        2);  % Add to file name
...
disp(' ');      
disp('-------------------------');  
disp('AVERAGING dB AND ISI SOURCES WITHIN SUBJECTS (MLR)'); 
disp(datetime)
disp('-------------------------');     
disp(' '); 
...
for p = 1:length(participant)
    ...
    for mode = 1:length(modality_data)
         for nopt = 1:length(source_noise_tags)
            folders = dir([root_dir_bs '/data/' participant{p} '/@intra/']);
            % Quietest
            results =  contains({folders.name},[modality_data{mode} '_' source_space{se} '_' source_noise_tags{nopt} '_' wave{w}]) & ... 
            ...
            for ex = 1:length(Exp_cond) 
                ...
                if ~isempty(sFiles)
                    ...
                    if delete_previous_file == 1
                        ...
                        % TYPE in the folder where we are gonna calculate it
                        folders_delete = dir([root_dir_bs '/data/' participant{p} '/@intra/']);
                        results_delete =  endsWith({folders_delete.name}, [modality_data{mode} '_' source_space{se} '_' source_noise_tags{nopt} '_' wave{w} '_' Exp_cond{ex} '.mat']); 
                        infolder_delete = find(results_delete);
                        if ~isempty(infolder_delete) % file exists, therefore delete it
                           delete([root_dir_bs '/data/' participant{p} '/@intra/' folders_delete(infolder_delete).name]);
                        end
                    ...
                    disp(' ');
                    disp('-------------------------');  
                    disp(['Average sources across db and ISI (' modality_data{mode} '_' source_space{se} '_' source_noise_tags{nopt} ') for ' participant{p} '_' wave{w}]);
                    disp(datetime)
                    disp(' ');
                    ...
                    % Process: Add tag
                    sFiles = bst_process('CallProcess', 'process_add_tag', sFiles, [], ...
                        'tag',           [modality_data{mode} '_' source_space{se} '_' source_noise_tags{nopt} '_' wave{w} '_' Exp_cond{ex}], ...
                        'output',        2);  % Add to file name (1 to add a tag)
...
clearvars('-except', initialVars{:});
disp 'DONE WITH AVERAGING SOURCES ACROSS dB and ISI (MLR)!!!'
disp(datetime)
toc
...
disp(' ');      
disp('-------------------------');  
disp('AVERAGING SOURCES ACROSS ALL CONDITIONS (MLR)'); 
disp(datetime)
disp('-------------------------');     
disp(' '); 
         ...
         for nopt = 1:length(source_noise_tags)
            ...
            if  ~isempty(sFiles)
                if delete_previous_file == 1
                    ...
                    % TYPE in the folder where we are gonna calculate it
                    folders_delete = dir([root_dir_bs '/data/' participant{p} '/@intra/']);
                    results_delete =  endsWith({folders_delete.name}, [modality_data{mode} '_' source_space{se} '_' source_noise_tags{nopt} '_' wave{w} '_ALL.mat']); 
                    infolder_delete = find(results_delete);
                    if ~isempty(infolder_delete) % file exists, therefore delete it
                       delete([root_dir_bs '/data/' participant{p} '/@intra/' folders_delete(infolder_delete).name]);
                    end
                ...
                disp(' ');
                disp('-------------------------');  
                disp(['Average sources across all conditions (' modality_data{mode} '_' source_space{se} '_' source_noise_tags{nopt} ') for ' participant{p} '_' wave{w}]);
                disp(datetime)
                disp(' ');
                ...
                % Process: Add tag
                sFiles = bst_process('CallProcess', 'process_add_tag', sFiles, [], ...
                    'tag',           [modality_data{mode} '_' source_space{se} '_' source_noise_tags{nopt} '_' wave{w} '_ALL'], ...
                    'output',        2);  % Add to file name (1 to add a tag)
...
clearvars('-except', initialVars{:});
disp 'DONE WITH AVERAGING SOURCES ACROSS ALL CONDITIONS (MLR)!!!'
disp(datetime)
toc

%% CORTICAL: Before CORTICAL GAVR source level, set to absolute and project to anatomy 
...
disp(' ');      
disp('-------------------------');  
disp('SET SOURCES TO ABSOLUTE AND PROJECTING TO COMMON ANATOMY (CORTEX MLR)'); 
disp(datetime)
disp('-------------------------');     
disp(' '); 
...
for w = 1:length(wave)
    for c = 1:length(condition)
         for nopt = 1:length(source_noise_tags)
             ...
             for p = 1:length(participant)
                folders = dir([root_dir_bs '/data/' participant{p} '/@intra/']);
                results =  contains({folders.name},[modality_data{mode} '_Surf_' source_noise_tags{nopt} '_' wave{w}]) & ...
                    endsWith({folders.name},[condition{c} '.mat']);
                infolder = find(results);
                if isempty(infolder) % In case, for instance, MLR files are not there yet
                    error(['No ' modality_data{mode} '_Surf_' source_noise_tags{nopt} '_' wave{w} '_' condition{c} ' file for participant' participant{p}]);
                ...
                disp(' ');
                disp('-------------------------');  
                disp(['Obtain absolute source values (' modality_data{mode} '_Surf_' source_noise_tags{nopt} ') for ' participant{p} '_' wave{w} '_' condition{c}]);
                disp(datetime)
                disp(' ');
                ...
                if delete_previous_file == 1
                    ...
                    folders_delete = dir([root_dir_bs '/data/' participant{p} '/@intra/']);
                    results_delete =  endsWith({folders_delete.name}, [modality_data{mode} '_Surf_' source_noise_tags{nopt} '_' wave{w} '_' condition{c} '_abs.mat']); 
                    infolder_delete = find(results_delete);
                    if ~isempty(infolder_delete) % file exists, therefore delete it
                       delete([root_dir_bs '/data/' participant{p} '/@intra/' folders_delete(infolder_delete).name]);
                    end
                ...
                if delete_previous_file == 1
                    ...
                    & endsWith({folders_delete.name}, [modality_data{mode} '_Surf_' source_noise_tags{nopt} '_' wave{w} '_' condition{c} '.mat']); 
                    infolder_delete = find(results_delete);
                    if ~isempty(infolder_delete) % file exists, therefore delete it
                       delete([root_dir_bs '/data/Group_analysis/@intra/' folders_delete(infolder_delete).name]);
                    end
                ...
                % Process: Add tag
                sFiles = bst_process('CallProcess', 'process_add_tag', sFiles, [], ...
                    'tag',           [participant{p} '_' modality_data{mode} '_Surf_' source_noise_tags{nopt} '_' wave{w} '_' condition{c}], ...
                    'output',        2);  % Add to file name (1 to add a tag)
    ...
    for ex = 1:length(Exp_cond)
         for nopt = 1:length(source_noise_tags)
             for p = 1:length(participant)
                ...
                infolder = find(results);
                if isempty(infolder) % In case, for instance, MLR files are not there yet
                    error(['No ' modality_data{mode} '_Surf_' source_noise_tags{nopt} '_' wave{w} '_' Exp_cond{ex} ' file for participant' participant{p}]);
                elseif size(infolder,2)> 1 % in case of more than one coincidence (should not happen), pick first
                   infolder = infolder(1);
                end
                ...
                disp(' ');
                disp('-------------------------');  
                disp(['Obtain absolute source values (' modality_data{mode} '_Surf_' source_noise_tags{nopt} ') for ' participant{p} '_' wave{w} '_' Exp_cond{ex}]);
                disp(datetime)
                disp(' ');
                ...
                if delete_previous_file == 1
                    ...
                    % abs present in the folder may be the old one
                    folders_delete = dir([root_dir_bs '/data/' participant{p} '/@intra/']);
                    results_delete =  endsWith({folders_delete.name}, [modality_data{mode} '_Surf_' source_noise_tags{nopt} '_' wave{w} '_' Exp_cond{ex} '_abs.mat']); 
                    infolder_delete = find(results_delete);
                    if ~isempty(infolder_delete) % file exists, therefore delete it
                       delete([root_dir_bs '/data/' participant{p} '/@intra/' folders_delete(infolder_delete).name]);
                    end
                ...
                if delete_previous_file == 1
                    ...
                    % results_average_200512_2003_Source_MEG_Regul_MLR_11_abs_2005_2005_Source_MEG_Regul_MLR_11
                    results_delete = contains({folders_delete.name},['_abs_' participant{p}])...
                    & endsWith({folders_delete.name}, [modality_data{mode} '_Surf_' source_noise_tags{nopt} '_' wave{w} '_' Exp_cond{ex} '.mat']); 
                    infolder_delete = find(results_delete);
                    if ~isempty(infolder_delete) % file exists, therefore delete it
                       delete([root_dir_bs '/data/Group_analysis/@intra/' folders_delete(infolder_delete).name]);
                    end
                ...
                % Process: Add tag
                sFiles = bst_process('CallProcess', 'process_add_tag', sFiles, [], ...
                    'tag',           [participant{p} '_' modality_data{mode} '_Surf_' source_noise_tags{nopt} '_' wave{w} '_' Exp_cond{ex}], ...
                    'output',        2);  % Add to file name (1 to add a tag)
...
for w = 1:length(wave)
     for nopt = 1:length(source_noise_tags)
         ...
         for p = 1:length(participant)
            ...
            infolder = find(results);
            if isempty(infolder) % In case, for instance, MLR files are not there yet
                error(['No ' modality_data{mode} '_Surf_' source_noise_tags{nopt} '_' wave{w} '_ALL file for participant' participant{p}]);
            elseif size(infolder,2)> 1 % in case of more than one coincidence (should not happen), pick first
               infolder = infolder(1);
            end
            ...
            error('for whatever reason, sFiles is empty, my friend...')
                ...
                disp(' ');
                disp('-------------------------');  
                disp(['Obtain absolute source values (' modality_data{mode} '_Surf_' source_noise_tags{nopt} ') for ' participant{p} '_' wave{w} '_ALL']);
                disp(datetime)
                disp(' ');
                ...
                if delete_previous_file == 1
                    ...
                    % abs present in the folder may be the old one
                    folders_delete = dir([root_dir_bs '/data/' participant{p} '/@intra/']);
                    results_delete =  endsWith({folders_delete.name}, [modality_data{mode} '_Surf_' source_noise_tags{nopt} '_' wave{w} '_ALL_abs.mat']); 
                    infolder_delete = find(results_delete);
                    if ~isempty(infolder_delete) % file exists, therefore delete it
                       delete([root_dir_bs '/data/' participant{p} '/@intra/' folders_delete(infolder_delete).name]);
                    end
                ...
                if delete_previous_file == 1
                    ...
                    % results_average_200512_2003_Source_MEG_Regul_MLR_11_abs_2005_2005_Source_MEG_Regul_MLR_11
                    results_delete = contains({folders_delete.name},['_abs_' participant{p}])...
                    & endsWith({folders_delete.name}, [modality_data{mode} '_Surf_' source_noise_tags{nopt} '_' wave{w} '_ALL.mat']); 
                    infolder_delete = find(results_delete);
                    if ~isempty(infolder_delete) % file exists, therefore delete it
                       delete([root_dir_bs '/data/Group_analysis/@intra/' folders_delete(infolder_delete).name]);
                    end
                ...
                % Process: Add tag
                sFiles = bst_process('CallProcess', 'process_add_tag', sFiles, [], ...
                    'tag',           [participant{p} '_' modality_data{mode} '_Surf_' source_noise_tags{nopt} '_' wave{w} '_ALL'], ...
                    'output',        2);  % Add to file name (1 to add a tag)
...
clearvars('-except', initialVars{:});
disp 'SOURCES TO ABSOLUTE AND PROJECTING TO COMMON ANATOMY (CORTEX MLR)!!!'
disp(datetime)
toc

end

%% CORTICAL GAVR source level
...
disp(' ');      
disp('-------------------------');  
disp('GAVR SOURCE DATA (CORTEX MLR)'); 
disp(datetime)
disp('-------------------------');
disp(' ');
    ...
    for c = 1:length(condition)
         for nopt = 1:length(source_noise_tags)
             ...
             for p = 1:length(participant)
                ...
                results =  endsWith({folders.name},[participant{p} '_' modality_data{mode} '_Surf_' source_noise_tags{nopt} '_' wave{w} '_' condition{c} '.mat']) ...
                    & ~contains({folders.name},'GAVR');
                infolder = find(results);
                if isempty(infolder) % In case, for instance, MLR files are not there yet
                    error(['No ' modality_data{mode} '_Surf_' source_noise_tags{nopt} '_' wave{w} '_' condition{c} ' file for participant' participant{p}]);
                elseif size(infolder,2)> 1 % in case of more than one coincidence (should not happen), pick first
                   error(['More than one ' modality_data{mode} '_Surf_' source_noise_tags{nopt} '_' wave{w} '_' condition{c} ' file for participant' participant{p}]);
                end
                ...
                disp(' ');
                disp('-------------------------');  
                disp(['GAVR sources ' modality_data{mode} '_Surf_' source_noise_tags{nopt} ' for ' wave{w} '_' condition{c}]);
                disp(datetime)
                disp(' ');
                ...
                % Process: Add tag
                sFiles = bst_process('CallProcess', 'process_add_tag', sFiles, [], ...
                    'tag',           ['GAVR_' modality_data{mode} '_Surf_' source_noise_tags{nopt} '_' wave{w} '_' condition{c}], ...
                    'output',        2);  % Add to file name (1 to add a tag)
...
for w = 1:length(wave)
    for ex = 1:length(Exp_cond)
         for nopt = 1:length(source_noise_tags)
             ...
             for p = 1:length(participant)
                folders = dir([root_dir_bs '/data/Group_analysis/@intra/']);
                results =  endsWith({folders.name},[participant{p} '_' modality_data{mode} '_Surf_' source_noise_tags{nopt} '_' wave{w} '_' Exp_cond{ex} '.mat']) ...
                    & ~contains({folders.name},'GAVR');
                infolder = find(results);
                if isempty(infolder) % In case, for instance, MLR files are not there yet
                    error(['No ' modality_data{mode} '_Surf_' source_noise_tags{nopt} '_' wave{w} '_' Exp_cond{ex} ' file for participant' participant{p}]);
                elseif size(infolder,2)> 1 % in case of more than one coincidence (should not happen), pick first
                   error(['More than one ' modality_data{mode} '_Surf_' source_noise_tags{nopt} '_' wave{w} '_' Exp_cond{ex} ' file for participant' participant{p}]);
                end
                ...
                disp(' ');
                disp('-------------------------');  
                disp(['GAVR sources ' modality_data{mode} '_Surf_' source_noise_tags{nopt} ' for ' wave{w} '_' Exp_cond{ex}]);
                disp(datetime)
                disp(' ');
                ...
                % Process: Add tag
                sFiles = bst_process('CallProcess', 'process_add_tag', sFiles, [], ...
                    'tag',           ['GAVR_' modality_data{mode} '_Surf_' source_noise_tags{nopt} '_' wave{w} '_' Exp_cond{ex}], ...
                    'output',        2);  % Add to file name (1 to add a tag)
...
for w = 1:length(wave)
     for nopt = 1:length(source_noise_tags)
         ...
         for p = 1:length(participant)
            folders = dir([root_dir_bs '/data/Group_analysis/@intra/']);
            results =  endsWith({folders.name},[participant{p} '_' modality_data{mode} '_Surf_' source_noise_tags{nopt} '_' wave{w} '_ALL.mat']) & ...
               ~contains({folders.name},'GAVR');
            infolder = find(results);
            if isempty(infolder) % In case, for instance, MLR files are not there yet
                error(['No ' modality_data{mode} '_Surf_' source_noise_tags{nopt} '_' wave{w} '_ALL file for participant' participant{p}]);
            elseif size(infolder,2)> 1 % in case of more than one coincidence (should not happen), pick first
               error(['More than one ' modality_data{mode} '_Surf_' source_noise_tags{nopt} '_' wave{w} '_ALL file for participant' participant{p}]);
            end
            ...
            disp(' ');
            disp('-------------------------');  
            disp(['GAVR sources ' modality_data{mode} '_Surf_' source_noise_tags{nopt} ' for ' wave{w} '_ALL']);
            disp(datetime)
            disp(' ');
            ...
            % Process: Add tag
            sFiles = bst_process('CallProcess', 'process_add_tag', sFiles, [], ...
                'tag',           ['GAVR_' modality_data{mode} '_Surf_' source_noise_tags{nopt} '_' wave{w} '_ALL'], ...
                'output',        2);  % Add to file name (1 to add a tag)
...
clearvars('-except', initialVars{:});
disp 'DONE WITH GAVR SOURCES (CORTEX MLR)!!!'
disp(datetime)
toc

end

%% VOLUME: Before VOLUME GAVR source level, set to absolute and project to anatomy 
...
disp(' ');      
disp('-------------------------');  
disp('SET SOURCES TO ABSOLUTE AND PROJECTING TO COMMON ANATOMY (VOLUME MLR)'); 
disp(datetime)
disp('-------------------------');     
disp(' '); 
...
for w = 1:length(wave)
    for c = 1:length(condition)
         for nopt = 1:length(source_noise_tags)
             ...
             for p = 1:length(participant)
                folders = dir([root_dir_bs '/data/' participant{p} '/@intra/']);
                results =  contains({folders.name},[modality_data{mode} '_Vol_' source_noise_tags{nopt} '_' wave{w}]) & ...
                    endsWith({folders.name},[condition{c} '.mat']);
                infolder = find(results);
                if isempty(infolder) % In case, for instance, MLR files are not there yet
                    error(['No ' modality_data{mode} '_Vol_' source_noise_tags{nopt} '_' wave{w} '_' condition{c} ' file for participant' participant{p}]);
                elseif size(infolder,2)> 1 % in case of more than one coincidence (should not happen), pick first
                   infolder = infolder(1);
                end
                ...
                disp(' ');
                disp('-------------------------');  
                disp(['Obtain absolute source values (' modality_data{mode} '_Vol_' source_noise_tags{nopt} ') for ' participant{p} '_' wave{w} '_' condition{c}]);
                disp(datetime)
                disp(' ');
                ...
                if delete_previous_file == 1
                    ...
                    % abs present in the folder may be the old one
                    folders_delete = dir([root_dir_bs '/data/' participant{p} '/@intra/']);
                    results_delete =  endsWith({folders_delete.name}, [modality_data{mode} '_Vol_' source_noise_tags{nopt} '_' wave{w} '_' condition{c} '_abs.mat']); 
                    infolder_delete = find(results_delete);
                    if ~isempty(infolder_delete) % file exists, therefore delete it
                       delete([root_dir_bs '/data/' participant{p} '/@intra/' folders_delete(infolder_delete).name]);
                    end
                ...
                if delete_previous_file == 1
                    ...
                    % results_average_200512_2003_Source_MEG_Regul_MLR_11_abs_2005_2005_Source_MEG_Regul_MLR_11
                    results_delete = contains({folders_delete.name},['_abs_' participant{p}])...
                    & endsWith({folders_delete.name}, [modality_data{mode} '_Vol_' source_noise_tags{nopt} '_' wave{w} '_' condition{c} '.mat']); 
                    infolder_delete = find(results_delete);
                    if ~isempty(infolder_delete) % file exists, therefore delete it
                       delete([root_dir_bs '/data/Group_analysis/@intra/' folders_delete(infolder_delete).name]);
                    end
                ...
                % Process: Add tag
                sFiles = bst_process('CallProcess', 'process_add_tag', sFiles, [], ...
                    'tag',           [participant{p} '_' modality_data{mode} '_Vol_' source_noise_tags{nopt} '_' wave{w} '_' condition{c}], ...
                    'output',        2);  % Add to file name (1 to add a tag)
    ...
    for ex = 1:length(Exp_cond)
         for nopt = 1:length(source_noise_tags)
             for p = 1:length(participant)
                ...
                infolder = find(results);
                if isempty(infolder) % In case, for instance, MLR files are not there yet
                    error(['No ' modality_data{mode} '_Vol_' source_noise_tags{nopt} '_' wave{w} '_' Exp_cond{ex} ' file for participant' participant{p}]);
                elseif size(infolder,2)> 1 % in case of more than one coincidence (should not happen), pick first
                   infolder = infolder(1);
                end
                ...
                disp(' ');
                disp('-------------------------');  
                disp(['Obtain absolute source values (' modality_data{mode} '_Vol_' source_noise_tags{nopt} ') for ' participant{p} '_' wave{w} '_' Exp_cond{ex}]);
                disp(datetime)
                disp(' ');
                ...
                if delete_previous_file == 1
                    ...
                    % abs present in the folder may be the old one
                    folders_delete = dir([root_dir_bs '/data/' participant{p} '/@intra/']);
                    results_delete =  endsWith({folders_delete.name}, [modality_data{mode} '_Vol_' source_noise_tags{nopt} '_' wave{w} '_' Exp_cond{ex} '_abs.mat']); 
                    infolder_delete = find(results_delete);
                    if ~isempty(infolder_delete) % file exists, therefore delete it
                       delete([root_dir_bs '/data/' participant{p} '/@intra/' folders_delete(infolder_delete).name]);
                    end
                ...
                if delete_previous_file == 1
                    ...
                    % results_average_200512_2003_Source_MEG_Regul_MLR_11_abs_2005_2005_Source_MEG_Regul_MLR_11
                    results_delete = contains({folders_delete.name},['_abs_' participant{p}])...
                    & endsWith({folders_delete.name}, [modality_data{mode} '_Vol_' source_noise_tags{nopt} '_' wave{w} '_' Exp_cond{ex} '.mat']); 
                    infolder_delete = find(results_delete);
                    if ~isempty(infolder_delete) % file exists, therefore delete it
                       delete([root_dir_bs '/data/Group_analysis/@intra/' folders_delete(infolder_delete).name]);
                    end
                ...
                % Process: Add tag
                sFiles = bst_process('CallProcess', 'process_add_tag', sFiles, [], ...
                    'tag',           [participant{p} '_' modality_data{mode} '_Vol_' source_noise_tags{nopt} '_' wave{w} '_' Exp_cond{ex}], ...
                    'output',        2);  % Add to file name (1 to add a tag)
...
for w = 1:length(wave)
     for nopt = 1:length(source_noise_tags)
         ...
         for p = 1:length(participant)
            ...
            infolder = find(results);
            if isempty(infolder) % In case, for instance, MLR files are not there yet
                error(['No ' modality_data{mode} '_Vol_' source_noise_tags{nopt} '_' wave{w} '_ALL file for participant' participant{p}]);
            elseif size(infolder,2)> 1 % in case of more than one coincidence (should not happen), pick first
               infolder = infolder(1);
            end
            ...
            error('for whatever reason, sFiles is empty, my friend...')
                ...
                disp(' ');
                disp('-------------------------');  
                disp(['Obtain absolute source values (' modality_data{mode} '_Vol_' source_noise_tags{nopt} ') for ' participant{p} '_' wave{w} '_ALL']);
                disp(datetime)
                disp(' ');
                ...
                if delete_previous_file == 1
                    ...
                    % abs present in the folder may be the old one
                    folders_delete = dir([root_dir_bs '/data/' participant{p} '/@intra/']);
                    results_delete =  endsWith({folders_delete.name}, [modality_data{mode} '_Vol_' source_noise_tags{nopt} '_' wave{w} '_ALL_abs.mat']); 
                    infolder_delete = find(results_delete);
                    if ~isempty(infolder_delete) % file exists, therefore delete it
                       delete([root_dir_bs '/data/' participant{p} '/@intra/' folders_delete(infolder_delete).name]);
                    end
                ...
                if delete_previous_file == 1
                    ...
                    % results_average_200512_2003_Source_MEG_Regul_MLR_11_abs_2005_2005_Source_MEG_Regul_MLR_11
                    results_delete = contains({folders_delete.name},['_abs_' participant{p}])...
                    & endsWith({folders_delete.name}, [modality_data{mode} '_Vol_' source_noise_tags{nopt} '_' wave{w} '_ALL.mat']); 
                    infolder_delete = find(results_delete);
                    if ~isempty(infolder_delete) % file exists, therefore delete it
                       delete([root_dir_bs '/data/Group_analysis/@intra/' folders_delete(infolder_delete).name]);
                    end
                ...
                % Process: Add tag
                sFiles = bst_process('CallProcess', 'process_add_tag', sFiles, [], ...
                    'tag',           [participant{p} '_' modality_data{mode} '_Vol_' source_noise_tags{nopt} '_' wave{w} '_ALL'], ...
                    'output',        2);  % Add to file name (1 to add a tag)
...
clearvars('-except', initialVars{:});
disp 'SOURCES TO ABSOLUTE AND PROJECTING TO COMMON ANATOMY (VOLUME MLR)!!!'
disp(datetime)
toc

end

%% VOLUME GAVR source level

if any(contains(source_space,'Vol'))

% ALSO TAKE THIS INTO ACCOUNT for se = 1:length(source_space) 

tic
disp(' ');      
disp('-------------------------');  
disp('GAVR SOURCE DATA (VOLUME MLR)'); 
disp(datetime)
disp('-------------------------');
disp(' ');
...
for w = 1:length(wave)
    for c = 1:length(condition)
         for nopt = 1:length(source_noise_tags)
             ...
             for p = 1:length(participant)
                folders = dir([root_dir_bs '/data/Group_analysis/@intra/']);
                results =  endsWith({folders.name},[participant{p} '_' modality_data{mode} '_Vol_' source_noise_tags{nopt} '_' wave{w} '_' condition{c} '.mat']) ...
                    & ~contains({folders.name},'GAVR');
                infolder = find(results);
                if isempty(infolder) % In case, for instance, MLR files are not there yet
                    error(['No ' modality_data{mode} '_Vol_' source_noise_tags{nopt} '_' wave{w} '_' condition{c} ' file for participant' participant{p}]);
                elseif size(infolder,2)> 1 % in case of more than one coincidence (should not happen), pick first
                   error(['More than one ' modality_data{mode} '_Vol_' source_noise_tags{nopt} '_' wave{w} '_' condition{c} ' file for participant' participant{p}]);
                end
                ...
                disp(' ');
                disp('-------------------------');  
                disp(['GAVR sources ' modality_data{mode} '_Vol_' source_noise_tags{nopt} ' for ' wave{w} '_' condition{c}]);
                disp(datetime)
                disp(' ');
                ...
                % Process: Add tag
                sFiles = bst_process('CallProcess', 'process_add_tag', sFiles, [], ...
                    'tag',           ['GAVR_' modality_data{mode} '_Vol_' source_noise_tags{nopt} '_' wave{w} '_' condition{c}], ...
                    'output',        2);  % Add to file name (1 to add a tag)
...
for w = 1:length(wave)
    for ex = 1:length(Exp_cond)
         for nopt = 1:length(source_noise_tags)
             ...
             for p = 1:length(participant)
                folders = dir([root_dir_bs '/data/Group_analysis/@intra/']);
                results =  endsWith({folders.name},[participant{p} '_' modality_data{mode} '_Vol_' source_noise_tags{nopt} '_' wave{w} '_' Exp_cond{ex} '.mat']) ...
                    & ~contains({folders.name},'GAVR');
                infolder = find(results);
                if isempty(infolder) % In case, for instance, MLR files are not there yet
                    error(['No ' modality_data{mode} '_Vol_' source_noise_tags{nopt} '_' wave{w} '_' Exp_cond{ex} ' file for participant' participant{p}]);
                elseif size(infolder,2)> 1 % in case of more than one coincidence (should not happen), pick first
                   error(['More than one ' modality_data{mode} '_Vol_' source_noise_tags{nopt} '_' wave{w} '_' Exp_cond{ex} ' file for participant' participant{p}]);
                end
                ...
                disp(' ');
                disp('-------------------------');  
                disp(['GAVR sources ' modality_data{mode} '_Vol_' source_noise_tags{nopt} ' for ' wave{w} '_' Exp_cond{ex}]);
                disp(datetime)
                disp(' ');
                ...
                % Process: Add tag
                sFiles = bst_process('CallProcess', 'process_add_tag', sFiles, [], ...
                    'tag',           ['GAVR_' modality_data{mode} '_Vol_' source_noise_tags{nopt} '_' wave{w} '_' Exp_cond{ex}], ...
                    'output',        2);  % Add to file name (1 to add a tag)
...
for w = 1:length(wave)
     for nopt = 1:length(source_noise_tags)
         ...
         for p = 1:length(participant)
            folders = dir([root_dir_bs '/data/Group_analysis/@intra/']);
            results =  endsWith({folders.name},[participant{p} '_' modality_data{mode} '_Vol_' source_noise_tags{nopt} '_' wave{w} '_ALL.mat']) & ...
               ~contains({folders.name},'GAVR');
            infolder = find(results);
            if isempty(infolder) % In case, for instance, MLR files are not there yet
                error(['No ' modality_data{mode} '_Vol_' source_noise_tags{nopt} '_' wave{w} '_ALL file for participant' participant{p}]);
            elseif size(infolder,2)> 1 % in case of more than one coincidence (should not happen), pick first
               error(['More than one ' modality_data{mode} '_Vol_' source_noise_tags{nopt} '_' wave{w} '_ALL file for participant' participant{p}]);
            end
            ...
            disp(' ');
            disp('-------------------------');  
            disp(['GAVR sources ' modality_data{mode} '_Vol_' source_noise_tags{nopt} ' for ' wave{w} '_ALL']);
            disp(datetime)
            disp(' ');
            ...
            % Process: Add tag
            sFiles = bst_process('CallProcess', 'process_add_tag', sFiles, [], ...
                'tag',           ['GAVR_' modality_data{mode} '_Vol_' source_noise_tags{nopt} '_' wave{w} '_ALL'], ...
                'output',        2);  % Add to file name (1 to add a tag)
...
clearvars('-except', initialVars{:});
disp 'DONE WITH GAVR SOURCES (VOLUME MLR)!!!'
disp(datetime)
toc
...
%% Extract event times from all conditions (for SINGLE TRIAL AMPLITUDES, MLR ONLY!!)

% This will overwrite previous logs, but it's necessary since MLR was
% computed using AMICA_10Hz files, which introduce an overall lag at the
% times (apparently it moves data and triggers toghether, so no problem in
% that sense)

tic
disp(' ');      
disp('-------------------------');  
disp('OBTAINING EVENT TIMES & ISI (FOR SINGLE TRIAL AMPLITUDES MLR)');  
disp(datetime)
disp('-------------------------');     
disp(' ');
...
for p = 1:length(participant)
    ...
    for s = 1:length(session)
        ...
        participant{p} session{s} '_Click*_tsss_AMICA_10Hz.vmrk']);
        ...
        for r = 1:length(runs_in_block)
            ...
            folders = dir([root_dir '/analysis/' participant{p}(1:4) session{s} '/']);
            results =  contains({folders.name},runs_in_block{r}) & endsWith({folders.name},'AMICA_10Hz.vmrk');
            infolder = find(results);
                if isempty(infolder)
                    continue
                end
...
% So here's the thing. For me to retrieve the amplitude values of the
% epochs analyzed in Brainstorm, I need to match the trial times in
% Brainstorm with the trial times I retrieve form the AMICA vrmk files,
% which are the ones loaded. However, in the vmrk AMICA files, triggers
% from sections of EEG/MEG that were deleted during the preprocessing
% aren't present. On the other hand, the vmrk files of the original
% fif files before AMICA contain all the triggers, since these were
% obtained before preprocessing. So I could use these vrmk files to obtain
% a log of ALL triggers present, including the ones that were removed at 
% preprocessing. BUT! there is a minor inconsistency (bigger one spotted 
% of 5ms) between the two versions of vmrk files so, before using the
% values extracted from the original non-AMICA vmrk files, I need to
% correct this inconsistency in order to match the absolute times properly
% with sweeps obtained from brainstorm (which were read from AMICA files)

% AN ALTERNATIVE TO THIS WOULD BE TO REDUCE THE THRESHOLD FOR MATCHING
% BRAINSTORM SWEEPS WITH LOG DATA (<10^-1 INSTEAD OF <10^-3, THAT IS 1
% DECIMAL INSTEAD OF 3, WHICH ANYWAY SHOULD WORK, IN CASE THIS GIVES
% TROUBLE). BUT BETTER TO BE PRECISE.
...
%% Extract complete log for correction (for SINGLE TRIAL AMPLITUDES, same for MLR and LLR)
...
%% Obtain averaged SINGLE TRIAL amplitudes (MLR)

% This is meant to be ran once subtraction has already been applied to the
% individual sweeps

% Take into account that this is sensor level (although could be used for
% source waveforms as well), but be careful about which blocks and sessions
% are used (if you delete some for source reasons, they won't be taken into
% account when extracting single trials and will be empty (NaN) rows).

tic
disp(' ');      
disp('-------------------------');  
disp('OBTAINING DATA TABLE FOR FOR SINGLE TRIAL AMPLITUDES (MLR)');  
disp(datetime)
disp('-------------------------');     
disp(' ');

% If MLR seg is not the protocol at this step, better stop
% if ~strcmp(root_dir_bs,'~/brainstorm_db/project_MLRraw')
%     error('root_dir_bs is not MLRraw, change it (because of retrigger issue)!')
% end
...
for p = 1:length(participant)
    eval(['load ''' root_dir '/Events/Single_trials/Latencies_' participant{p} '_original.mat'''])
    ...
    for s = 1:length(session)
        ...
        for b = 1:length(block)
            for mode = 1:length(modality_data) % EEG and MEG
                for c = 1:length(condition)
                eval(['ncs_' participant{p} '_' modality_data{mode} '_' condition{c} session{s} '_' block{b} '_MLR = 0;'])    
                folders = dir([root_dir_bs '/data/' participant{p} '/']); % Will be MLRseg here
                results = contains({folders.name},['MLR_' condition{c} session{s} '_' block{b} '_normal']);
                infolder = find(results);
                    if isempty(infolder)
                        continue % should not be a problem unless all conditions are bad for a block, but in this case that block is not probably in the block list
                    ...
                    if ~exist([root_dir '/Events/BadTrials/MLR/' modality_data{mode} '/' participant{p} '/' folders(infolder).name '/BadTrials.mat'], 'file')
                        continue % if this folder does not have bad trials corresponding, better discard these sweeps
                    end
                    load([root_dir '/Events/BadTrials/MLR/' modality_data{mode} '/' participant{p} '/' folders(infolder).name '/BadTrials.mat']);
                    
                    disp(' ');      
                    disp('-------------------------');  
                    disp(['Extracting MLR single trial amplitudes ' modality_data{mode} '_' participant{p} '_' session{s} '_' block{b} '_' condition{c}]);
                    disp(' ');  
                    ...
                    for j= 1:length(trial_count) % for every trial
                        ...
                        % latency absolute value (for match with log data)
                        trial_time = str2double(History{x,y+1}(brack +1: com -1));
                        trial_time = trial_time + 0.05; % For MLR              
                        ...
                        trial_run = History{cell_import}(init_run_name+3:end_run_name-1); % gives the two digit number of the run
                        % Check if any Log file is empty before looking into it
                        eval(['empty_logs = isempty(Ori_Log_' condition{c} '_' participant{p} session{s} '_r' trial_run ');'])
                        if empty_logs == 1 % if the log is empty
                            continue
                        end
                        % find position in Log      
                        eval(['ident_pos = find(abs(trial_time  - Ori_Log_' condition{c} '_' participant{p} session{s} '_r' trial_run '(:,1))<10^-2);']) % could be even one
                        if isempty(ident_pos)
                            eval(['ncs_' participant{p} '_' modality_data{mode} '_' condition{c} session{s} '_' block{b} '_MLR = ncs_' participant{p} '_' modality_data{mode} '_' condition{c} session{s} '_' block{b} '_MLR + 1;'])
                            continue
                        end
                        % Previous category info
                        eval(['prev_cat = Ori_Log_' condition{c} '_' participant{p} session{s} '_r' trial_run '(ident_pos,3);'])
                        % avoid rare random categories that I don't expect (like '8' or '' or 'boundary'), I rather do this
                        ...
                        eval(['prev_lat = Ori_Log_' condition{c} '_' participant{p} session{s} '_r' trial_run '(ident_pos,2);'])
                        ...
                        if strcmp(modality_data{mode}, 'EEG')
                            for comp_MLR = 1:length(components_MLR)
                                eval(['EEG_' components_MLR{comp_MLR} '= trial_amplitudes(chan_list_EEG,fix(((baseline*1500) + (Time_wind_' components_MLR{comp_MLR} '(1)*1500))):fix(((baseline*1500) + (Time_wind_' components_MLR{comp_MLR} '(2)*1500))));']) 
                                eval(['EEG_' components_MLR{comp_MLR} '= EEG_' components_MLR{comp_MLR} '.*1000000;']) % Transform from Volts to microVolts
                                % Next two columns lines in case we wanna do average across sensors
                                % Delete any empty channels before average
%                                 eval(['EEG_' components_MLR{comp_MLR}  '= EEG_' components_MLR{comp_MLR}  '(any(EEG_' components_MLR{comp_MLR} ',2),:);'])
%                                 eval(['EEG_' components_MLR{comp_MLR} '= mean(EEG_' components_MLR{comp_MLR} ',1);']) 
                                eval(['EEG_' components_MLR{comp_MLR} '= mean(EEG_' components_MLR{comp_MLR} ');'])
                            end
                            
                            if isempty(Single_trial_amplitudes_EEG_MLR)                             
                                row_pos_EEG = 1;
                            else
                                row_pos_EEG = size(Single_trial_amplitudes_EEG_MLR,1) + 1;
                            end  
                            ...
                            if ~isempty(BadTrials)   
                                if any(contains(BadTrials, [dir_trial(position).name]))
                                    for comp_MLR = 1:length(components_MLR) % N1, P2, etc
                                        eval(['EEG_' components_MLR{comp_MLR} '= ''NaN'';'])
                                    end
                                end
                            end
                            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                            
                            Single_trial_amplitudes_EEG_MLR{row_pos_EEG,1} = j;
                            Single_trial_amplitudes_EEG_MLR{row_pos_EEG,2} = participant{p};  
                            Single_trial_amplitudes_EEG_MLR{row_pos_EEG,3} = session{s};    
                            Single_trial_amplitudes_EEG_MLR{row_pos_EEG,4} = block{b};
                            Single_trial_amplitudes_EEG_MLR{row_pos_EEG,5} = str2double(trial_run);
                            Single_trial_amplitudes_EEG_MLR{row_pos_EEG,6} = trial_time;
                            Single_trial_amplitudes_EEG_MLR{row_pos_EEG,7} = str2double(condition{c}(2)); % 1 (65dB), 2 (75dB), 3 (85dB)
                            Single_trial_amplitudes_EEG_MLR{row_pos_EEG,8} = prev_lat;
                            Single_trial_amplitudes_EEG_MLR{row_pos_EEG,9} = EEG_P0;
                            Single_trial_amplitudes_EEG_MLR{row_pos_EEG,10} = EEG_Na;
                            Single_trial_amplitudes_EEG_MLR{row_pos_EEG,11} = EEG_Pa;
                            Single_trial_amplitudes_EEG_MLR{row_pos_EEG,12} = EEG_Nb;
                            Single_trial_amplitudes_EEG_MLR{row_pos_EEG,13} = EEG_P50;
                            
                        elseif strcmp(modality_data{mode}, 'MEG')
                            for comp_MLR = 1:length(components_MLR) 
                                eval(['Meg_Mag_' components_MLR{comp_MLR} '= trial_amplitudes(chan_list_MEG_MAG,fix(((baseline*1500) + (Time_wind_' components_MLR{comp_MLR} '(1)*1500))):fix(((baseline*1500) + (Time_wind_' components_MLR{comp_MLR} '(2)*1500))));']) 
                                eval(['Meg_Mag_' components_MLR{comp_MLR} '= Meg_Mag_' components_MLR{comp_MLR} '.*1000000000000000;']) % Tesla to fT
                                % Next two columns lines in case we wanna do average across sensors
                                % Delete any empty channels before average
%                                 eval(['Meg_Mag_' components_MLR{comp_MLR}  '= Meg_Mag_' components_MLR{comp_MLR}  '(any(Meg_Mag_' components_MLR{comp_MLR} ',2),:);'])
%                                 eval(['Meg_Mag_' components_MLR{comp_MLR} '= mean(Meg_Mag_' components_MLR{comp_MLR} ',1);']) 
                                eval(['Meg_Mag_' components_MLR{comp_MLR} '= mean(Meg_Mag_' components_MLR{comp_MLR} ');'])
                            end
                            
%                           row_pos = ((s-1)*45) + ((b-1)*15) + c; % index position in table 
                            if isempty(Single_trial_amplitudes_MEG_MLR)                             
                                row_pos_MEG = 1;
                            else
                                row_pos_MEG = size(Single_trial_amplitudes_MEG_MLR,1) + 1;
                            end
                            ...
                            if ~isempty(BadTrials)   
                                if any(contains(BadTrials, [dir_trial(position).name]))
                                    for comp_MLR = 1:length(components_MLR) % N1, P2, etc
%                                         eval(['Meg_Grad_' components_MLR{comp_MLR} '= ''NaN'';'])
                                        eval(['Meg_Mag_' components_MLR{comp_MLR} '= ''NaN'';'])
                                    end
                                end
                            end
                            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                            
                            Single_trial_amplitudes_MEG_MLR{row_pos_MEG,1} = j;
                            Single_trial_amplitudes_MEG_MLR{row_pos_MEG,2} = participant{p};  
                            Single_trial_amplitudes_MEG_MLR{row_pos_MEG,3} = session{s};    
                            Single_trial_amplitudes_MEG_MLR{row_pos_MEG,4} = block{b};
                            Single_trial_amplitudes_MEG_MLR{row_pos_MEG,5} = str2double(trial_run);
                            Single_trial_amplitudes_MEG_MLR{row_pos_MEG,6} = trial_time;
                            Single_trial_amplitudes_MEG_MLR{row_pos_MEG,7} = str2double(condition{c}(2)); % 1 (65dB), 2 (75dB), 3 (85dB)
                            Single_trial_amplitudes_MEG_MLR{row_pos_MEG,8} = prev_lat;
                            Single_trial_amplitudes_MEG_MLR{row_pos_MEG,9} = Meg_Mag_P0;
                            Single_trial_amplitudes_MEG_MLR{row_pos_MEG,10} = Meg_Mag_Na;
                            Single_trial_amplitudes_MEG_MLR{row_pos_MEG,11} = Meg_Mag_Pa;
                            Single_trial_amplitudes_MEG_MLR{row_pos_MEG,12} = Meg_Mag_Nb;
                            Single_trial_amplitudes_MEG_MLR{row_pos_MEG,13} = Meg_Mag_P50;
                        end   
                    end
                end
            end
            ...
            block_rows_EEG = find(strcmp({Single_trial_amplitudes_EEG_MLR{:,2}}, participant{p}) ...
            & strcmp({Single_trial_amplitudes_EEG_MLR{:,3}}, session{s})...
             & strcmp({Single_trial_amplitudes_EEG_MLR{:,4}}, block{b})); 
            % Delimite current block in matrix
            current_block_EEG = Single_trial_amplitudes_EEG_MLR(block_rows_EEG,:);    
            ...
            Single_trial_amplitudes_EEG_MLR(block_rows_EEG,:) = current_block_EEG;
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            
            %%%%%%%%%%%%%%%% MEG %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            % find rows of this block (just for this session and subject)
            block_rows_MEG = find(strcmp({Single_trial_amplitudes_MEG_MLR{:,2}}, participant{p}) ...
            & strcmp({Single_trial_amplitudes_MEG_MLR{:,3}}, session{s})...
             & strcmp({Single_trial_amplitudes_MEG_MLR{:,4}}, block{b})); 
            % Delimite current block in matrix
            current_block_MEG = Single_trial_amplitudes_MEG_MLR(block_rows_MEG,:);    
            ...
            Single_trial_amplitudes_MEG_MLR(block_rows_MEG,:) = current_block_MEG;
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        end
    end
...
Amp_singl_trials_EEG_MLR = array2table(Single_trial_amplitudes_EEG_MLR,'VariableNames',colNames_EEG_MLR);
Amp_singl_trials_MEG_MLR = array2table(Single_trial_amplitudes_MEG_MLR,'VariableNames',colNames_MEG_MLR);
writetable(Amp_singl_trials_EEG_MLR, [root_dir '/Events/Single_trials/Amp_singl_trials_EEG_MLR.txt'], 'Delimiter','\t')
writetable(Amp_singl_trials_MEG_MLR, [root_dir '/Events/Single_trials/Amp_singl_trials_MEG_MLR.txt'], 'Delimiter','\t')

% Reset modality_data variable to its original self
modality_data = {'EEG','MEG','both_mod'};

clearvars('-except', initialVars{:});
disp 'DONE WITH OBTAINING DATA TABLE FOR FOR SINGLE TRIAL AMPLITUDES!!!'
disp(datetime)
toc

%% Fill table with missing trials for MLR (rejected at preprocessing or lost)

tic
disp(' ');      
disp('-------------------------');  
disp('FILL SINGLE TRIAL TABLE WITH MISSING TRIALS (MLR)');  
disp(datetime)
disp('-------------------------');     
disp(' ');

% If MLR seg is not the protocol at this step, better stop
% if ~strcmp(root_dir_bs,'~/brainstorm_db/project_MLRseg')
%     error('root_dir_bs is not MLRseg, change it!')
% end
...
Amp_singl_trials_MLR = readtable([root_dir '/Events/Single_trials/Amp_singl_trials_' modality_data{mode} '_MLR.txt']);
for p = 1:length(participant)
    ...
    for s = 1:length(session)
        ...
        for r = 1:length(runs_in_block)
            ...
            disp(' ');      
            disp('-------------------------');  
            disp(['Filling single trial matrix  ' modality_data{mode} '_' participant{p} '_' session{s} '_' runs_in_block{r} 'MLR']);
            disp(' '); 
            ...
            if isempty(rr)
                ...
                if number_of_run == 1 && s == 1 % first run and session of this subject
                    ...
                    rp_1 = find(contains(Amp_singl_trials_MLR.subjID,participant{p-1}));
                ...
                elseif number_of_run == 1 && s ~= 1 % so there is a previous session to check
                    ...
                    rs_1 = find(contains(Amp_singl_trials_MLR.sessionNr(rp),session{s-1})); %#ok<*FNDSB>
                    rs_1 = rp(rs_1); 
                    row_add_run = rs_1(end);
                    block = 'b1'; % exception because if the whole block is
                    % not in session_block array it's gonna 
                else % This is not the first run, so use last as reference
                    rr_1 = find(Amp_singl_trials_MLR.run(rs) == number_of_run-1);
                    rr_1 = rs(rr_1);
...
%                         previous_block = Amp_singl_trials_MLR.blockNr{row_add_run};
...
%                         block = ['b' num2str(previous_block)];
                ...
                for i = 1:rows_dataArray
                    ...
                    Amp_singl_trials_MLR = Amp_singl_trials_MLR([1:row_add_run,row_add_run:end],:);
                end
                for i = 1:rows_dataArray
                    % Second, add the info in each of created rows
                    Amp_singl_trials_MLR.TrialNr(row_add_run+i) = i;
                    Amp_singl_trials_MLR.subjID{row_add_run+i} = participant{p}; % not needed
                    Amp_singl_trials_MLR.sessionNr{row_add_run+i} = session{s};
                    Amp_singl_trials_MLR.blockNr{row_add_run+i} = block;
                    Amp_singl_trials_MLR.run(row_add_run+i) = number_of_run;
                    curr_abs_time = dataArray_full(i,2)/1500; % from samples to s
                    Amp_singl_trials_MLR.absTime(row_add_run+i) = curr_abs_time;
                    ...
                    Amp_singl_trials_MLR.clickIntensity(row_add_run+i) = current_dB;
                    ...
                    if i == 1 % it's the first trial from this run
                        Amp_singl_trials_MLR.ISI(row_add_run+i)= NaN;
                    else
                        current_ISI = curr_abs_time - Amp_singl_trials_MLR.absTime(row_add_run+i-1);
                        % Add value into the matrix
                        Amp_singl_trials_MLR.ISI(row_add_run+i) = current_ISI; % in s
                    end
                    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

                    % Add NaN in amplitude values
                    for comp_MLR = 1:length(components_MLR) % Will do for every column of components
                        eval(['Amp_singl_trials_MLR.' modality_data{mode} '_' components_MLR{comp_MLR} '(row_add_run+i)= NaN;'])
                    end
                end
                continue
            end
            ...
            for i = 1:length(dataArray_full)
                ...
                rv = find(abs(curr_abs_time - Amp_singl_trials_MLR.absTime(rr))<10^-2); % could be one if found many inconsistencies
                ...
                [minValue,closestIndex] = min(abs(curr_abs_time - Amp_singl_trials_MLR.absTime(rr)));
                ...
                if  curr_abs_time > Amp_singl_trials_MLR.absTime(closestIndex)
                ...
                Amp_singl_trials_MLR = Amp_singl_trials_MLR([1:closestIndex,closestIndex:end],:); 
                % Now in the right position 
                
                Amp_singl_trials_MLR.TrialNr(position) = 999; % Since it will be corrected later
                % Not needed to update subject, session, block or run since they should be the same
                Amp_singl_trials_MLR.absTime(position) = curr_abs_time; % in s
                % Determine dB based on trigger code of current value:
                current_dB = num2str(dataArray_full(i,1));
                try
                    current_dB = str2double(current_dB(2));
                ...
                % Add value into the matrix
                Amp_singl_trials_MLR.clickIntensity(position) = current_dB; 
                % Determine ISI using current and previous abs time
                % provided it's not first trial of run (in which case we
                % cannot compare with abs times from previous run)
                if i == 1 % it's the first trial from this run
                    Amp_singl_trials_MLR.ISI(position)= NaN;
                else
                    current_ISI = curr_abs_time - Amp_singl_trials_MLR.absTime(position-1);
                    % Add value into the matrix
                    Amp_singl_trials_MLR.ISI(position) = current_ISI; % in s
                end
                % Add NaN in amplitude values
                for comp_MLR = 1:length(components_MLR) % Will do for every column of components
                    eval(['Amp_singl_trials_MLR.' modality_data{mode} '_' components_MLR{comp_MLR} '(position)= NaN;'])
                end
            ...
            Amp_singl_trials_MLR.TrialNr(rr) = 1:length(Amp_singl_trials_MLR.TrialNr(rr));   
        end    
    end 
end

if strcmp(modality_data{mode},'EEG')
    Amp_singl_trials_EEG_MLR = Amp_singl_trials_MLR;
    writetable(Amp_singl_trials_EEG_MLR, [root_dir '/Events/Single_trials/Amp_singl_trials_EEG_MLR.txt'])
    clear('Amp_singl_trials_EEG_MLR'); % to free space
elseif strcmp(modality_data{mode},'MEG')
    Amp_singl_trials_MEG_MLR = Amp_singl_trials_MLR;
    writetable(Amp_singl_trials_MEG_MLR, [root_dir '/Events/Single_trials/Amp_singl_trials_MEG_MLR.txt'])
    clear('Amp_singl_trials_MEG_MLR'); % to free space
end
...
clearvars('-except', initialVars{:});
disp 'DONE WITH FILLING SINGLE TRIAL TABLE WITH MISSING TRIALS (MLR)!!!'
disp(datetime)
toc

%% Fix three sessions with inverted click intensities MLR

tic
disp(' ');      
disp('-------------------------');  
disp('FIX THREE SESSIONS WITH INVERTED CLICK INTENSITIES (MLR)');  
disp(datetime)
disp('-------------------------');     
disp(' ');
...
% Find 2005D louder
A = ismember(Amp_singl_trials_MLR.subjID,'2005');
B = ismember(Amp_singl_trials_MLR.sessionNr,'D');
C = find(Amp_singl_trials_MLR.clickIntensity == 3);
count = 1;
for i = 1:size(Amp_singl_trials_MLR,1)
    if A(i)&& B(i) && sum(i == C)
...
A = ismember(Amp_singl_trials_MLR.subjID,'2005');
B = ismember(Amp_singl_trials_MLR.sessionNr,'D');
C = find(Amp_singl_trials_MLR.clickIntensity == 1);
count = 1;
for i = 1:size(Amp_singl_trials_MLR,1)
    if A(i)&& B(i) && sum(i == C)
...
% Find 2238G louder
A = ismember(Amp_singl_trials_MLR.subjID,'2238');
B = ismember(Amp_singl_trials_MLR.sessionNr,'G');
C = find(Amp_singl_trials_MLR.clickIntensity == 3);
count = 1;
for i = 1:size(Amp_singl_trials_MLR,1)
    if A(i)&& B(i) && sum(i == C)
...
% Find 2235H louder
A = ismember(Amp_singl_trials_MLR.subjID,'2235');
B = ismember(Amp_singl_trials_MLR.sessionNr,'H');
C = find(Amp_singl_trials_MLR.clickIntensity == 3);
count = 1;
for i = 1:size(Amp_singl_trials_MLR,1)
    if A(i)&& B(i) && sum(i == C)
...
for i = 1:length(coincident_rows_2005D_louder)
    ...
    Amp_singl_trials_MLR.clickIntensity(line) = 1;
end
for i = 1:length(coincident_rows_2005D_quieter)
    line = coincident_rows_2005D_quieter(i);
    Amp_singl_trials_MLR.clickIntensity(line) = 3;
end
% 2238G
...
clearvars('-except', initialVars{:});
disp 'DONE WITH FIXING THREE SESSIONS WITH INVERTED CLICK INTENSITIES (MLR)!!!'
disp(datetime)
toc

%% Eliminate any remaining sweeps above threshold (MLR)

tic
disp(' ');      
disp('-------------------------');  
disp('ELIMINATING ANY REMAINING SWEEPS ABOVE THRESHOLD (MLR)');  
disp(datetime)
disp('-------------------------');     
disp(' ');
...
if strcmp(modality_data{mode},'EEG')
    ...
    A = find(Amp_singl_trials_MLR.EEG_P0 > 50);
    B = find(Amp_singl_trials_MLR.EEG_P0 < -50);
    C = find(Amp_singl_trials_MLR.EEG_Na > 50);
    D = find(Amp_singl_trials_MLR.EEG_Na < -50);
    E = find(Amp_singl_trials_MLR.EEG_Pa > 50);
    F = find(Amp_singl_trials_MLR.EEG_Pa < -50);
    G = find(Amp_singl_trials_MLR.EEG_Nb > 50);
    H = find(Amp_singl_trials_MLR.EEG_Nb < -50);
    I = find(Amp_singl_trials_MLR.EEG_P50 > 50);
    J = find(Amp_singl_trials_MLR.EEG_P50 < -50);

    % Find rows with any of these coincidences
    combined_list = [A;B;C;D;E;F;G;H;I;J];
    final_row_list = unique(combined_list);

    % Replace values in these rows by NaN
    for i = 1:length(final_row_list)
        line = final_row_list(i);
        Amp_singl_trials_MLR.EEG_P0(line) = NaN;
        Amp_singl_trials_MLR.EEG_Na(line) = NaN;
        Amp_singl_trials_MLR.EEG_Pa(line) = NaN;
        Amp_singl_trials_MLR.EEG_Nb(line) = NaN;
        Amp_singl_trials_MLR.EEG_P50(line) = NaN;
    end
...
elseif strcmp(modality_data{mode},'MEG')
    ...
    A = find(Amp_singl_trials_MLR.MEG_P0 > 50);
    B = find(Amp_singl_trials_MLR.MEG_P0 < -50);
    C = find(Amp_singl_trials_MLR.MEG_Na > 50);
    D = find(Amp_singl_trials_MLR.MEG_Na < -50);
    E = find(Amp_singl_trials_MLR.MEG_Pa > 50);
    F = find(Amp_singl_trials_MLR.MEG_Pa < -50);
    G = find(Amp_singl_trials_MLR.MEG_Nb > 50);
    H = find(Amp_singl_trials_MLR.MEG_Nb < -50);
    I = find(Amp_singl_trials_MLR.MEG_P50 > 50);
    J = find(Amp_singl_trials_MLR.MEG_P50 < -50);
    ...
    for i = 1:length(final_row_list)
        line = final_row_list(i);
        Amp_singl_trials_MLR.MEG_P0(line) = NaN;
        Amp_singl_trials_MLR.MEG_Na(line) = NaN;
        Amp_singl_trials_MLR.MEG_Pa(line) = NaN;
        Amp_singl_trials_MLR.MEG_Nb(line) = NaN;
        Amp_singl_trials_MLR.MEG_P50(line) = NaN;
    end
...
clearvars('-except', initialVars{:});
disp 'DONE ELIMINATING ANY REMAINING SWEEPS ABOVE THRESHOLD (MLR)!!!'
disp(datetime)
toc

%%
% %% Correct event times in SINGLE TRIAL matrix (MLR) to fit complete event log 
% 
% % IN THE CASE OF MLR, THERE IS A CONSISTENT BUT LARGE (300 MS)
% % DIFFERENCE BETWEEN VALUES AT RAW AND AMICA_10HZ VMRK FILES THAT MUST BE
% % ACCOUNTED FOR. WHEN RETRIEVING VALUES FROM BRAINSTORM WE HAD
% % TO USE THE TRIGGER TIMES IN AMICA_10HZ, OTHERWISE WE COULD NOT MATCH
% % WHAT WAS IN BRAINSTORM TO RETRIEVE THE AMPLITUDES, BUT NOW THAT WE HAVE THE MATRIX 
% % WE SHOULD CORRECT THE ABS_TIMES IN THERE TO MAKE IT FIT WITH THE ORIGINAL
% % RAW VMRK COMPLETE LOG FILES (Events/complete_event_log files)
% % WE COULD DO LIKE IN LLR AND ADJUST THE RAW VMRK TO HAVE THE SAME TIMING
% % THAN THE AMICA_10HZ ONES IN THIS CASE, THEN WE COULD EASILY FILL THE GAPS
% % IN THE LAST STEP THE SAME WAY WE DID FOR LLR. BUT IT'S BETTER TO HAVE THE
% % SAME REFERENCE OF ABSOLUTE TIMES IN LLR AND MLR MATRICES, SO THAT'S WHY
% % WE ARE DOING THIS
% % THAT LAG SEEMS TO BE BECAUSE OF A LARGE BOUNDARY AT THE BEGGINING OF EACH FILE 
% % (MAYBE FILTER AT EEGLAB?)
% 
% tic
% disp(' ');      
% disp('-------------------------');  
% disp('ADJUSTING ABS TIMES IN MLR SINGLE TRIAL MATRIX');  
% disp(datetime)
% disp('-------------------------');     
% disp(' ');
% 
...
%             disp(' ');      
%             disp('-------------------------');  
%             disp(['Fixing MLR single trial matrix times for  ' modality_data{mode} '_' participant{p} '_' session{s} '_' runs_in_block{r}]);
%             disp(' '); 
%             
%             % Load dataArray_full
%             load ([root_dir '/Events/complete_event_log/' participant{p}(1:4) session{s} '/'...
...
%             dataArray_full = rmmissing(dataArray_full);
%             
%             %Load dataArray_full_mlr
%             load ([root_dir '/Events/complete_event_log/' participant{p}(1:4) session{s} '/'...
%             participant{p} session{s} '_' runs_in_block{r} '10Hz.mat']);
%             % Remove rows with NaN 
%             dataArray_full_mlr = rmmissing(dataArray_full_mlr);
%             
%             % Test if there is a mismatch between the two files (sometimes
%             % there is not)
%             
%             
%             % Retrieve number of run:
...
% if strcmp(modality_data{mode},'EEG')
%     Amp_singl_trials_EEG_MLR = Amp_singl_trials_MLR;
%     writetable(Amp_singl_trials_EEG_MLR, [root_dir '/Events/Single_trials/Amp_singl_trials_EEG_MLR_CORRECTED.txt'])
%     clear('Amp_singl_trials_EEG_MLR'); % to free space
% elseif strcmp(modality_data{mode},'MEG')
%     Amp_singl_trials_MEG_MLR = Amp_singl_trials_MLR;
%     writetable(Amp_singl_trials_MEG_MLR, [root_dir '/Events/Single_trials/Amp_singl_trials_MEG_MLR_CORRECTED.txt'])
%     clear('Amp_singl_trials_MEG_MLR'); % to free space
% end
% 
% end
...
% 
% clearvars('-except', initialVars{:});
% disp 'DONE WITH ADJUSTING ABS TIMES IN MLR SINGLE TRIAL MATRIX!!!'
% disp(datetime)
% toc

%% Probably will never be used but just in case: calculate CORTICAL forward model (HAS TO BE IN 98)
% Compute leadfields only once per each participant, session and block
tic
disp(' ');      
disp('-------------------------');
disp('COMPUTING FORWARD MODEL (EEG and MEG) FOR EACH PARTICIPANT, SESSION AND BLOCK (MLR)');
disp(datetime)
disp('-------------------------');
disp(' ');
...
for p = 1:length(participant)
    ...
    for s = 1:length(session)
        ...
        for b = 1:length(block)
            ...
            if delete_previous_file == 1
                % check if there is already a headmodel OF THIS TYPE in the folder
                if isfile([root_dir_bs '/data/' participant{p} '/MLR_11' session{s} '_' block{b} '_normal/headmodel_surf_os_meg.mat'])
                    delete([root_dir_bs '/data/' participant{p} '/MLR_11' session{s} '_' block{b} '_normal/headmodel_surf_os_meg.mat'])
                end
            ...
            if delete_previous_file == 1
                % check if there is already a headmodel OF THIS TYPE in the folder
                if isfile([root_dir_bs '/data/' participant{p} '/MLR_11' session{s} '_' block{b} '_normal/headmodel_surf_duneuro.mat'])
                    delete([root_dir_bs '/data/' participant{p} '/MLR_11' session{s} '_' block{b} '_normal/headmodel_surf_duneuro.mat'])
                end
                % head model = headmodel_surf_os_meg.mat
                % source = contains KERNEL endswith _Source_Regul_MEG.mat                 
            end
...
for p = 1:length(participant)
    ...
    for s = 1:length(session)
        ...
        % If folder does not exists or does not contain the head model, don't even try
        if ~exist([root_dir_bs '/data/' participant{p} '/MLR_11' session{s} '_' block{1} '_normal'], 'dir');continue;end
        if ~exist([root_dir_bs '/data/' participant{p} '/MLR_11' session{s} '_' block{1} '_normal/headmodel_surf_duneuro.mat'],'file')
            error([' no EEG forward model in ' participant{p} '_MLR_11' session{s} '_' block{1} '_normal']);
        end
        ...
        % The EEG forward model from first block
        file_name_leadfield = [root_dir_bs '/data/' participant{p} '/MLR_11' session{s} '_' block{1} '_normal/headmodel_surf_duneuro.mat'];
        ...
        for b = 2:length(block)
            ...
            if exist([root_dir_bs '/data/' participant{p} '/MLR_11' session{s} '_' block{b} '_normal/'],'dir')
                copyfile(file_name_leadfield,[root_dir_bs '/data/' participant{p} '/MLR_11' session{s} '_' block{b} '_normal/headmodel_surf_duneuro.mat']);
            end
        end
    end
end
...
for p = 1:length(participant)
    ...
    for s = 1:length(session)
        ...
        for b = 1:length(block)
            ...
            % one of the forward models to merge is not present, go to next block
            if ~exist([root_dir_bs '/data/' participant{p} '/MLR_11' session{s} '_' block{b} '_normal'], 'dir'); continue; end
            if ~exist([root_dir_bs '/data/' participant{p} '/MLR_11' session{s} '_' block{b} '_normal/headmodel_surf_duneuro.mat'],'file'); continue; end
            if ~exist([root_dir_bs '/data/' participant{p} '/MLR_11' session{s} '_' block{b} '_normal/headmodel_surf_os_meg.mat'],'file'); continue; end

            EEG_leadfield = [root_dir_bs '/data/' participant{p} '/MLR_11' session{s} '_' block{b} '_normal/headmodel_surf_duneuro.mat'];
            MEG_leadfield = [root_dir_bs '/data/' participant{p} '/MLR_11' session{s} '_' block{b} '_normal/headmodel_surf_os_meg.mat'];
            ...
            % If stated, find and delete any previous combined leadfield 
            if delete_previous_file == 1
                if isfile([root_dir_bs '/data/' participant{p} '/MLR_11' session{s} '_' block{b} '_normal/headmodel_surf_duneuro_os_meg.mat'])
                    delete([root_dir_bs '/data/' participant{p} '/MLR_11' session{s} '_' block{b} '_normal/headmodel_surf_duneuro_os_meg.mat'])
                end              
            end
...
for p = 1:length(participant)
    ...
    for s = 1:length(session)
        ...
        for b = 1:length(block)
            ...
            if ~exist([root_dir_bs '/data/' participant{p} '/MLR_11' session{s} '_' block{b} '_normal/headmodel_surf_duneuro_os_meg.mat'],'file') 
                error(['no combined forward model for ' participant{p} '_MLR_11' session{s} '_' block{b} '_normal'])
            end
            % If folder exists but does not contain the combined head model, don't even try
            ...
            % This name is the one we obtain for the MEG overlapping spheres
            file_name_leadfield = [root_dir_bs '/data/' participant{p} '/MLR_11' session{s} '_' block{b} '_normal/headmodel_surf_duneuro_os_meg.mat'];
            ...
            for c = 2:length(condition)
                ...
                if delete_previous_file == 1
                    % check if there is already a headmodel OF THIS
                    % TYPE in the folder where we are going to copy the file
                    if isfile([root_dir_bs '/data/' participant{p} '/MLR_' condition{c} session{s} '_' block{b} '_normal/headmodel_surf_duneuro_os_meg.mat'])
                        delete([root_dir_bs '/data/' participant{p} '/MLR_' condition{c} session{s} '_' block{b} '_normal/headmodel_surf_duneuro_os_meg.mat'])
                    end
                    % head model = headmodel_surf_os_meg.mat
                    % source = contains KERNEL endswith _Source_Regul_MEG.mat                 
                end
                if exist([root_dir_bs '/data/' participant{p} '/MLR_' condition{c} session{s} '_' block{b} '_normal/'],'dir')
                    copyfile(file_name_leadfield,[root_dir_bs '/data/' participant{p} '/MLR_' condition{c} session{s} '_' block{b} '_normal/headmodel_surf_duneuro_os_meg.mat']);
                end
            end
        end
    end
...
save([root_dir '/Sources/No_head_model_created.mat'],'No_head_model_created');
clearvars('-except', initialVars{:});
disp 'DONE WITH COMPUTING FORWARD MODEL FOR EACH PARTICIPANT, SESSION AND BLOCK (MLR)!!!'
disp(datetime)
toc
